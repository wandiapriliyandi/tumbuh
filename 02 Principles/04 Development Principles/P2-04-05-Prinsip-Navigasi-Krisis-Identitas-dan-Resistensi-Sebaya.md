...
```mermaid
flowchart TD
    subgraph DinamikaIdentitasRemajaPesantren["DINAMIKA PSIKOSOSIAL REMAJA ASRAMA"]
        Krisis["KRISIS IDENTITAS & HORMON PUBERTAS:<br/>Pencarian jati diri, butuh pengakuan, & sensitif terhadap penolakan."]
        
        Salah["ARAH SALAH (PEMBIARAN TANPA PENDAMPINGAN):<br/>Terbentuk geng asrama toksik $\rightarrow$ Budaya senioritas, flexing, & perundungan."]
        
        Benar["ARAH TUMBUH (REKAYASA BUDAYA UKHUWAH):<br/>• Pendampingan dialogis asatidz sebagai sahabat (In Loco Parentis).<br/>• Penyaluran bakat ke kompetisi ilmiah & dakwah.<br/>• Komunitas kamar inklusif & dewan pelayan adab."]
        
        Krisis -.-> Salah
        Krisis ==> Benar
        
        Salah --> Kehancuran["SANTRI TRAUMA & PUTUS PONDOK"]
        Benar --> Unggul["INSAN ADABI BERINTEGRITAS & PEMIMPIN PERADABAN"]
    end
```
...
```mermaid
graph TD
    A["Gugatan Pesimisme Remaja (Pihak A):<br/>'Anak remaja itu memang sumber masalah dan nakal; harus dihukum keras biar kapok!'"]
    --> B1["Tinjauan Hadits Shahih Bukhari No. 660:<br/>'Syabbun nasya'a fi 'ibadatillahi Ta'ala' (Pemuda yang tumbuh dalam ibadah kepada Allah adalah 1 dari 7 golongan yang dinaungi Arsy di hari kiamat)."]
    A --> B2["Tinjauan Atsar Nasihat Sayyidina Ali bin Abi Thalib RA:<br/>'Ajarilah anakmu pada 7 tahun pertama lewat bermain, 7 tahun kedua lewat disiplin adab, dan 7 TAHUN KETIGA SEBAGAI SAHABAT (KHAWIKHUM).'"]
    A --> B3["Tinjauan Sirah Usamah bin Zaid RA:<br/>Rasulullah SAW mempercayai Usamah bin Zaid memimpin pasukan perang terbesar pada usia 18 tahun."]
    B1 & B2 & B3 --> C["Titik Temu Konsensus:<br/>Remaja adalah aset agung peradaban; metode pendampingannya adalah kemitraan dialogis."]
```
...
```mermaid
flowchart TD
    subgraph ProtokolPencegahanGengAsrama["PROTOKOL PENCEGAHAN POLARISASI GENG ASRAMA"]
        RotasiKamar["1. ROTASI KOMPOSISI KAMAR INKLUSIF (SETIAP TAHUN AJARAN)<br/>Mencegah pengelompokan homogen berbasis kedaerahan/asal daerah."]
        
        TradisiNampan["2. PEMBUDAYAAN MAKAN BERSAMA SATU NAMPAN (TA'AM JAMĀ'Ī)<br/>Menghilangkan sekat senioritas & kasta sosial antar-santri."]
        
        CircleMalam["3. HALAQAH UKHUWAH & CHECK-IN EMOSI PEKANAN<br/>Musyrif memandu sesi keterbukaan hati & penyelesaian friksi antarteman."]
        
        ProyekBersama["4. PROYEK KHIDMAH & OLAHRAGA KOLEKTIF ASRAMA<br/>Membangun rasa saling membutuhkan melalui gotong royong nyata."]
        
        RotasiKamar --> TradisiNampan --> CircleMalam --> ProyekBersama
    end
```
...
```mermaid
flowchart TD
    subgraph SOPPembubaranGengRestoratif["SOP DEKONSTRUKSI GENG ASRAMA RESTORATIF"]
        DeteksiDini["1. DETEKSI DINI OLEH MUSYRIF & LOGBOOK PBIS<br/>Mendeteksi pola kumpul tertutup, bahasa sandi rahasia, atau flexing kelompok."]
        
        WawancaraPrivat["2. WAWANCARA MOTIVASIONAL EMPAT MATA (BUKAN SIDANG UMUM)<br/>Musyrif menggali motif terdalam anggota geng tanpa nada menghakimi."]
        
        KonferensiRestoratif["3. RESTORATIVE JUSTICE CIRCLE DENGAN ADIK KELAS<br/>Mediasi ishlah untuk memulihkan hubungan dan mengembalikan barang/uang yang diambil."]
        
        RedistribusiPeran["4. REDISTRIBUSI PERAN POSITIF DI ASRAMA<br/>Mantan anggota geng dilantik menjadi Panitia Khidmah Sosial Asrama."]
        
        DeteksiDini --> WawancaraPrivat --> KonferensiRestoratif --> RedistribusiPeran
    end
```
