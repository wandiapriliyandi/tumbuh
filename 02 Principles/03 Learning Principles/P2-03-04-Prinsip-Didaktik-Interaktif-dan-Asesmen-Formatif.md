...
```mermaid
flowchart TD
    subgraph TransformasiDidaktikKelas["TRANSFORMASI DIDAKTIK KELAS PESANTREN"]
        MonologKering["CERAMAH MONOLOG PASIF & ASESMEN VONIS (LAMA)<br/>• Guru membaca teks 90 menit; 80% santri mengantuk & pasif.<br/>• Guru tidak tahu siapa yang paham & siapa yang bingung.<br/>• Vonis nilai angka merah di akhir semester tanpa bimbingan."]
        
        InteraktifTUMBUH["DIDAKTIK DIALOGIS NABAWI & ASESMEN FORMATIF (TUMBUH)<br/>• Struktur Kagan Cooperative Learning (100% santri aktif serempak).<br/>• Asesmen formatif harian (Exit Tickets & Mini-Whiteboards).<br/>• Umpan balik berdaya (Actionable Feedback) membimbing tuntas."]
        
        MonologKering ==>|REVOLUSI PEDAGOGI INTERAKTIF| InteraktifTUMBUH
    end
```
...
```mermaid
graph TD
    A["Gugatan Monolog Kaku (Pihak A):<br/>'Guru itu wibawanya tinggi kalau duduk diam membaca kitab dan santri tidak boleh banyak tanya atau berdiskusi di kelas!'"]
    --> B1["Tinjauan Sirah Nabawiyyah (Hadits Mu'awiyah bin Al-Hakam HR. Muslim):<br/>'Ma ra'aytu mu'alliman qablahu wala ba'dahu ahsana ta'liman minhu' (Aku tidak pernah melihat guru yang lebih baik cara mengajarnya daripada Rasulullah SAW)."]
    A --> B2["Tinjauan Hadits Shahih Bukhari 64:<br/>Metode dialog interaktif Rasulullah SAW: 'Atadruna manil muflis?' (Tahukah kalian siapakah orang yang bangkrut itu?)."]
    A --> B3["Tinjauan Kitab Adab al-Mu'allimin Ibnu Sahnun (w. 256 H):<br/>Kewajiban guru menyimak bacaan setiap murid secara bergiliran dan menguji pemahamannya."]
    B1 & B2 & B3 --> C["Titik Temu Konsensus:<br/>Didaktik interaktif, dialogis, & kasih sayang adalah metode pengajaran asli Rasulullah SAW."]
```
...
```mermaid
graph TD
    subgraph EmpatPilarPIESKagan["4 PRINSIP KETERLIBATAN KELAS KAGAN (PIES FRAMEWORK)"]
        P["1. POSITIVE INTERDEPENDENCE (Ketergantungan Positif)<br/>Santri saling membutuhkan dalam kelompok; sukses satu santri adalah sukses bersama."]
        
        I["2. INDIVIDUAL ACCOUNTABILITY (Tanggung Jawab Individu)<br/>Setiap santri wajib mampu menjelaskan hasil diskusi mandiri tanpa bergantung pada teman."]
        
        E["3. EQUAL PARTICIPATION (Partisipasi Berkeadilan)<br/>Setiap santri mendapatkan alokasi waktu bicara yang setara (misal: 1 menit bergantian)."]
        
        S["4. SIMULTANEOUS INTERACTION (Interaksi Serempak)<br/>Dalam metode Think-Pair-Share, 50% santri di kelas berbicara & berpikir secara bersamaan!"]
...
```mermaid
graph TD
    subgraph LimaStrategiAsesmenFormatifWiliam["5 STRATEGI KUNCI ASESMEN FORMATIF (DYLAN WILIAM)"]
        S1["1. MENGKLARIFIKASI KRITERIA SUKSES BELAJAR<br/>Menjelaskan tujuan & rubrik pemahaman yang jelas di awal sesi."]
        
        S2["2. MEREKAYASA DISKUSI PEMBUKTIAN BELAJAR EFEKTIF<br/>Mengajukan pertanyaan pemantik & polling kelas cepat (Mini-Whiteboards)."]
        
        S3["3. MEMBERIKAN UMPAN BALIK BERDAYA (ACTIONABLE FEEDBACK)<br/>Komentar tertulis yang memandu santri melangkah maju, bukan vonis angka."]
        
        S4["4. MENGAKTIFKAN SANTRI SEBAGAI SUMBER BELAJAR SEBAYA<br/>Peer-Assessment & bimbingan belajar teman sebangku (Peer Tutoring)."]
        
        S5["5. MENGAKTIFKAN SANTRI SEBAGAI PEMILIK BELAJARNYA SENDIRI<br/>Self-Assessment & pemantauan target pribadi (Metakognisi SRL)."]
...
```mermaid
graph LR
    subgraph TigaTeknikAsesmenFormatifCepat["3 TEKNIK ASESMEN FORMATIF HARIAN KELAS TUMBUH"]
        ExitTicket["1. EXIT TICKETS (Karcis Keluar 5 Menit)<br/>Santri menulis jawaban 1 soal kunci di kertas kecil sebelum bel istirahat berbunyi."]
        
        ColdCalling["2. COLD CALLING RAMAH (Tanpa Acung Jari)<br/>Guru memanggil nama santri secara acak dengan nada hangat untuk mengecek pemahaman merata."]
        
        MiniBoard["3. MINI-WHITEBOARDS (Papan Tulis Mini Santri)<br/>Seluruh santri menulis I'rab/jawaban di papan kecil & mengangkatnya serempak saat aba-aba."]
        
        ExitTicket --- ColdCalling --- MiniBoard
    end
```
...
| :--- | :--- | :--- | :---: |
| **1. Think-Pair-Share** | 1. Santri berpikir mandiri (1 Menit).<br/>2. Berdiskusi berpasangan dengan teman sebangku (2 Menit).<br/>3. Berbagi hasil nalar ke forum kelas. | Melatih kemandirian nalar, komunikasi ukhuwah, dan keberanian berpendapat. | Sesi pembuka saat menganalisis kaidah baru.[^9] |
...
```mermaid
graph TD
    subgraph MatriksAsesmenFormatifPesantren["5 STRATEGI ASESMEN FORMATIF DI PESANTREN"]
        A1["1. KEJELASAN TARGET: Menuliskan 'Capaian Adab & Materi Hari Ini' di sudut papan tulis."]
        A2["2. BUKTI PEMAHAMAN: Menggunakan Mini-Whiteboards untuk melihat jawaban 35 santri dalam 5 detik."]
        A3["3. UMPAN BALIK PANDUAN: Memberikan 2 Pujian Usaha + 1 Petunjuk Perbaikan (Two Stars & A Wish)."]
        A4["4. BELAJAR SEBAYA: Mengaktifkan Peer Tutoring (Santri Mutqin membimbing santri yang butuh bantuan)."]
        A5["5. REGULASI MANDIRI: Santri mengisi Checklist Pemahaman Pribadi di akhir modul pembelajaran."]
...
```mermaid
flowchart TD
    F1["1. IDENTIFIKASI CAPAIAN POSITIF (STAR 1 & STAR 2)<br/>Guru mengapresiasi aspek yang sudah dikerjakan dengan benar: 'Analisis kedudukan Mubtada' sangat tepat'."]
    
    F2["2. IDENTIFIKASI AREA PERBAIKAN KONKRET (A WISH)<br/>Guru menunjukkan titik kekeliruan secara presisi: 'Perhatikan harakat akhir Khabar Syibhul Jumlah'."]
    
    F3["3. PEMBERIAN PERTANYAAN PEMANDU / LANGKAH TINDAK LANJUT<br/>Guru menuliskan pertanyaan: 'Bagaimanakah tanda I'rab jika Khabar-nya berupa Isim Mutsanna?'."]
    
    F4["4. DEDIKASI WAKTU PERBAIKAN DI KELAS (DIRT TIME - 5 MENIT)<br/>Santri langsung membaca umpan balik & merevisi lembar jawabannya saat itu juga."]
