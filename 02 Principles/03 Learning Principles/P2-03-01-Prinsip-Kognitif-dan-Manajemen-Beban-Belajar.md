...
```mermaid
flowchart TD
    subgraph TransformasiDidaktikKognitif["TRANSFORMASI METODOLOGI DIDAKTIK KELAS"]
        PolaCeramahLama["CERAMAH CEPAT & VONIS MORAL (LAMA)<br/>• Menjejalkan rumus tanpa contoh bertahap.<br/>• Papan tulis semrawut & bentakan guru (High Extraneous Load).<br/>• Santri pusing, bingung, & divonis kurang ikhlas."]
        
        PolaCLTTUMBUH["DIDAKTIK KOGNITIF & DUAL CODING (TUMBUH)<br/>• Chunking: Materi dipecah menjadi unit-unit kecil.<br/>• Dual Coding: Penjelasan lisan dipadu bagan visual.<br/>• Worked-Example Effect: Demonstrasi langkah nalar."]
        
        PolaCeramahLama ==>|DIREKONSTRUKSI MENJADI| PolaCLTTUMBUH
    end
```
...
```mermaid
flowchart TD
    subgraph ArsitekturKognitifTUMBUH["ARSITEKTUR MANAJEMEN BEBAN KOGNITIF TUMBUH"]
        
        K1_Sayap["1. REDUKSI BEBAN PENGGANGGU (Eliminate Extraneous Load)<br/>Menghilangkan instruksi semrawut, papan tulis berantakan, & ketakutan bentakan guru."]
        
        K2_Sayap["2. PENGELOLAAN BEBAN ASLI MATERI (Manage Intrinsic Load)<br/>Strategi Chunking materi bertahap, prasyarat konsep, & penjadwalan teratur."]
        
        K3_Sayap["3. OPTIMALISASI SKEMA PRODUKTIF (Maximize Germane Load)<br/>Metode Dual Coding (Bagan Visual + Lisan), Worked-Example, & latihan reflektif."]
...
```mermaid
flowchart TD
    subgraph SOPStrukturPembelajaran["STRUKTUR PEMBELAJARAN 45 MENIT RAMAH KOGNISI"]
        Apersepsi["1. APERSEPSI & KONEKSI AWAL (5 Menit): Mengaitkan Materi Hari Ini dengan Skema Memori Lama"]
        --> EksplanasiModel["2. EKSPLANASI & WORKED-EXAMPLE (15 Menit): Guru Mendemonstrasikan Konsep via Dual Coding"]
        --> PraktikTerarah["3. PRAKTIK TERARAH BERPASANGAN (15 Menit): Santri Berlatih Bersama Teman Sebaya (Guidance Fading)"]
        --> RefleksiFormatif["4. ASESMEN TIKET KELUAR & REFLEKSI (10 Menit): Santri Menuliskan 1 Poin Pemahaman Inti"]
    end
```
