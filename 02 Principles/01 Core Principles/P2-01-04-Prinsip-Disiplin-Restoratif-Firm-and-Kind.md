...
```mermaid
flowchart TD
    subgraph TransformasiFirmAndKind["TRANSFORMASI PARADIGMA KEDISIPLINAN ASRAMA"]
        PolaEkstremLama["DIKOTOMI BINER SALAH (LAMA)<br/>• Otoriter: Memukul & membentak (memicu trauma).<br/>• Permisif: Membiarkan pelanggaran (memicu anarki).<br/>• Gagal membangun regulasi diri mandiri santri."]
        
        PolaFirmKindTUMBUH["DISIPLIN RESTORATIF FIRM & KIND (TUMBUH)<br/>• Firm: Aturan & batas syariat ditegakkan tanpa kompromi.<br/>• Kind: Cara menyampaikannya penuh kasih sayang (Ar-Rifq).<br/>• Connection Before Correction: Kesadaran taubat sejati."]
        
        PolaEkstremLama ==>|DIREKONSTRUKSI MENJADI| PolaFirmKindTUMBUH
    end
```
...
```mermaid
flowchart TD
    subgraph ArsitekturFirmKindTUMBUH["ARSITEKTUR DISIPLIN RESTORATIF FIRM & KIND"]
        
        FK1_Sayap["1. KETEGASAN BATASAN SYAR'I (Firm Boundaries)<br/>Standar adab & SOP ditegakkan pasti, konsisten, & tanpa pembiaran kelalaian."]
        
        FK2_Sayap["2. KELEMBUTAN CARA NABAWIB (Kind Delivery)<br/>Komunikasi empat mata, nada suara tenang, bebas makian, & penuh empati."]
        
        FK3_Sayap["3. RESTITUSI ISHLAH AL-BAIN (Restorative Repair)<br/>Fokus pada pemulihan hubungan rusak, tanggung jawab logis, & lembaran bersih."]
        
        FK1_Sayap <===> FK2_Sayap <===> FK3_Sayap
    end
```
...
```mermaid
flowchart TD
    subgraph ProtokolDialogRestoratif["PROTOKOL DIALOG RESTORATIF 4 TAHAP"]
        Regulate["1. REGULASI EMOSI (Regulate): Musyrif Menenangkan Diri & Santri dari Kepanikan/Kemarahan"]
        --> Relate["2. MEMBANGUN KONEKSI (Relate): Menatap Mata, Nada Hangat, & Mendengar Penjelasan Santri"]
        --> Reason["3. PENALARAN LOGIS (Reason): Menyadarkan Dampak Perbuatan & Menemukan Akar Masalah"]
        --> Repair["4. PERBAIKAN NYATA (Repair): Menyepakati Tindakan Konsekuensi Logis Restoratif"]
    end
```
...
Penerapan konsep **P2-01-04: PRINSIP DISIPLIN RESTORATIF FIRM AND KIND (FIRM & KIND RESTORATIVE DISCIPLINE)** di ekosistem pesantren TUMBUH memerlukan pemahaman multidimensional yang memadukan khazanah turats syariat dan konsensus sains pendidikan modern:
