...
```mermaid
flowchart TD
    subgraph KontinuumTadarrujIstiqamah["KONTINUUM TADARRUJ DAN ISTIQAMAH TUMBUH"]
        T1["1. FASE T1: TA'ARUF & KEMAMPUAN DASAR (Bulan 1-3)<br/>Fokus: Rasa aman psikologis, bangun tepat waktu, & adab kamar dasar"]
        
        T2["2. FASE T2-T3: TADRIB & TAFAHUM (Bulan 4-12)<br/>Fokus: Penambahan rawatib, tilawah mandiri, & pemahaman dalil adab"]
        
        T3["3. FASE T4: TAHWIL & ISTIQAMAH MELEKAT (Tahun ke-2+)<br/>Fokus: Karakter otonom spontan, memimpin adik kelas (Tahap 7 Penggerak)"]
        
        T1 ==>|Konsistensi Pembiasaan 66-Hari| T2
        T2 ==>|Penghayatan Batin Muraqabatullah| T3
    end
```
...
```mermaid
graph TD
    A["Gugatan Perfeksionisme Kaku (Pihak A):<br/>'Santri masuk pondok harus langsung 100% disiplin sempurna; tidak ada toleransi untuk proses adaptasi!'"]
    --> B1["Tinjauan Logika Falasi:<br/>All-or-Nothing Fallacy memicu kegagalan sistemik & keputusasaan santri."]
    A --> B2["Tinjauan Kaidah Turats:<br/>Man rama al-'ilma jumlatan dzahaba 'anhu jumlatan (Menuntut sekaligus niscaya hilang sekaligus)."]
    A --> B3["Tinjauan Psikologi Perilaku:<br/>Beban perubahan terlalu drastis memicu penolakan bawah sadar (Psychological Resistance)."]
    B1 & B2 & B3 --> C["Titik Temu Konsensus:<br/>Tadarruj adalah jalan fitrah menuju kematangan karakter permanen."]
```
...
```mermaid
graph TD
    A["Gugatan Historis (Pihak A):<br/>'Mengapa Allah Yang Maha Kuasa tidak menurunkan Al-Qur'an sekaligus dalam 1 malam?'"]
    --> B1["Tinjauan Nash QS. Al-Furqan 32:<br/>Li nutsabbita bihi fu'adaka wa rattalnahu tartila (Meneguhkan kalbu Rasul & umat secara bertahap)."]
    A --> B2["Tinjauan Atsar Sayyidah Aisyah ra:<br/>Ayat surga & neraka diturunkan lebih dahulu; jika larangan khamr turun duluan, orang menolak Islam."]
    A --> B3["Tinjauan Pedagogi Ilahiah:<br/>Fondasi keimanan & cinta Allah dibangun mendahului pembebanan hukum syariat."]
    B1 & B2 & B3 --> C["Titik Temu Konsensus:<br/>Tadarruj adalah metodologi tarbiyah Ilahiah yang paling agung."]
```
...
```mermaid
graph TD
    A["Gugatan Neurosains Pembiasaan (Pihak A):<br/>'Berapa lama waktu biologis yang dibutuhkan otak santri untuk mengubah suatu adab menjadi kebiasaan otomatis?'"]
    --> B1["Tinjauan Riset Phillippa Lally (UCL, 2010):<br/>Pembentukan kebiasaan otomatis (Habit Automaticity) rata-rata membutuhkan 66 hari konsisten."]
    A --> B2["Tinjauan Filosofi Kaizen & Atomic Habits (James Clear):<br/>Perbaikan 1% setiap hari (1% Better Every Day) melipatgandakan hasil 37 kali lipat dalam 1 tahun."]
    A --> B3["Tinjauan Sirkuit Basal Ganglia:<br/>Pengulangan harian memindahkan kendali adab dari Prefrontal Cortex ke Basal Ganglia."]
    B1 & B2 & B3 --> C["Titik Temu Konsensus:<br/>Tadarruj selaras secara presisi dengan sains neuroplastisitas pembiasaan adab."]
```
...
```mermaid
graph TD
    A["Gugatan Paradigma Ibadah (Pihak A):<br/>'Karamah itu adalah bisa terbang di udara atau berjalan di atas air; ibadah rutin biasa itu hal sepele!'"]
    --> B1["Tinjauan Tasawuf Sunni (Abu Ali al-Juzjani):<br/>Al-Istiqamatu a'zhamu karamah (Konsistensi adab adalah karamah sejati)."]
    A --> B2["Tinjauan Hadits Shahih Bukhari 6464:<br/>Ahabbu al-a'mali ilallahi adwamuha wa in qalla (Amalan paling dicintai Allah adalah yang paling konsisten meski sedikit)."]
    A --> B3["Tinjauan Bahaya Ghuluw (Ekstremitas Spiritual):<br/>Ibadah berlebihan yang memicu burnout dilarang oleh Rasulullah SAW."]
    B1 & B2 & B3 --> C["Titik Temu Konsensus:<br/>Istiqamah dalam adab harian adalah puncak capaian santri TUMBUH."]
```
...
```mermaid
flowchart TD
    Gejala["SANTRI MENUNJUKKAN GEJALA FUTUWR (LESU, MURUNG, HAFALAN MANDEK)"]
    --> Identifikasi["TAHAP 1: IDENTIFIKASI AKAR KELESUAN (Musyrif / BK)<br/>Apakah karena kelelahan fisik, homesickness, kejenuhan kognitif, atau masalah keluarga?"]
    
    Identifikasi --> Jeda["TAHAP 2: PEMBERIAN JEDA RESTORATIF (CO-REGULATION)<br/>Penyesuaian target sementara; memfasilitasi tidur cukup & olahraga relaksasi."]
    
    Jeda --> Dialog["TAHAP 3: DIALOG TAFAKKUR DARI HATI KE HATI<br/>Mendengarkan curahan hati tanpa menghakimi; mengingatkan kembali niat awal lillahi ta'ala."]
    
    Dialog --> Rebound["TAHAP 4: REBOUND & GRADUAL RE-ENGAGEMENT<br/>Memulai kembali dengan target micro-habits bertahap hingga semangat pulih penuh."]
```
...
```mermaid
flowchart TD
    A["Gugatan Standarisasi Kurikulum (Pihak A):<br/>'Bagaimana menyusun roadmap pentahapan adab santri yang terukur dari hari pertama hingga lulus pondok?'"]
    --> B1["Tahap T1: Ta'aruf (Orientasi & Adaptasi Aman / Bulan 1-3)<br/>Fokus: Rasa aman, mengenal musyrif, adab tidur & kebersihan dasar."]
...
```mermaid
flowchart TD
    Day1["HARI KE-1: SANTRI MEMULAI TANTANGAN ISTIQAMAH ADAB"]
    --> CheckDaily["PENCATATAN HARIAN LOGBOOK PBIS OLEH MUSYRIF / MENTOR<br/>(Target: Kehadiran shaf pertama, kamar rapi, tutur kata santun)"]
    
    CheckDaily --> Day20{"EVALUASI HARI KE-20 (FASE TENGAH)"}
    Day20 -- KONSISTEN --> Lanjut["LANJUTKAN MENUJU HARI KE-40"]
    Day20 -- ADA KHILAF 1 HARI --> TwoDayRule["APLIKASI THE TWO-DAY RULE: Evaluasi penyebab malam itu, perbaiki besok pagi tanpa reset nol!"]
    TwoDayRule --> Lanjut
    
    Lanjut --> Day40{"HARI KE-40 TERCAPAI PENUH?"}
    Day40 -- YA (KONSISTENSI MELEKAT) --> Award["PENGANUGERAHAN PIN & SERTIFIKAT 'BINTANG ISTIQAMAH MA'HAD'<br/>(Diumumkan di Apel Akbar & Dicatat di Portofolio Karakter)"]
```
