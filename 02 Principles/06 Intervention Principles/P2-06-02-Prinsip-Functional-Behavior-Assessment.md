...
```mermaid
flowchart TD
    subgraph TransformasiDiagnosisPerilaku["TRANSFORMASI DIAGNOSIS PERILAKU SANTRI"]
        PolaButaLama["KEBUTAAN FUNGSIONAL & HUKUMAN GEJALA (LAMA)<br/>• Santri gaduh langsung dipukul/diusir.<br/>• Tanpa tahu ia gaduh karena takut baca kitab.<br/>• Siklus masalah berulang & dendam batin."]
        
        PolaFBATUMBUH["FUNCTIONAL BEHAVIOR ASSESSMENT (TUMBUH)<br/>• Kaidah Al-Umuru bi Maqashidiha & Model ABC.<br/>• Bedah fungsi: Escape/Avoid vs Obtain/Access.<br/>• Ajarkan Replacement Behavior & Rencana BIP."]
        
        PolaButaLama ==>|DIREKONSTRUKSI MENJADI| PolaFBATUMBUH
    end
```
...
```mermaid
flowchart TD
    subgraph ArsitekturFBATUMBUH["ARSITEKTUR FUNCTIONAL BEHAVIOR ASSESSMENT (FBA)"]
        
        FBA1_Sayap["1. ANALISIS RANTAI MODEL ABC (Antecedent, Behavior, Consequence)<br/>Mengidentifikasi pemicu lingkungan, bentuk tindakan nyata, & konsekuensi penguat."]
        
        FBA2_Sayap["2. PEMETAAN 2 FUNGSI MOTIF (Escape/Avoid vs Obtain/Access)<br/>Menemukan akar kebutuhan batin: menghindari rasa malu atau mencari pengakuan sosial."]
        
        FBA3_Sayap["3. RENCANA AKSI BIP & REPLACEMENT BEHAVIOR (Behavior Intervention Plan)<br/>Modifikasi lingkungan pemicu, melatih keterampilan adab pengganti, & penguatan 4:1."]
        
        FBA1_Sayap <===> FBA2_Sayap <===> FBA3_Sayap
    end
```
...
```mermaid
flowchart TD
    subgraph SOPPelaksanaanFBA["SOP PELAKSANAAN FBA TIM BK"]
        RujukanKasus["1. RUJUKAN KASUS: Penerimaan Data Rujukan Santri Tier 3 dari Musyrif Asrama"]
        --> ObservasiABC["2. OBSERVASI ABC (3 Hari): Pengamatan Pola Pemicu & Konsekuensi di Asrama & Kelas"]
        --> WawancaraDiagnostik["3. WAWANCARA DIAGNOSTIK: Dialog Empatik Menyelami Niat & Beban Batin Santri"]
        --> PerumusanBIP["4. PENYUSUNAN BIP: Merumuskan Rencana Aksi Perilaku & Melatih Replacement Skill"]
        --> EvaluasiBerkala["5. REVIEW DWI-MINGGUAN: Memantau Penurunan Perilaku Masalah via Logbook PBIS"]
    end
```
