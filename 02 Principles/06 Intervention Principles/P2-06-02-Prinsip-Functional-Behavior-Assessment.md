...
```mermaid
flowchart TD
    subgraph TransformasiAnalisisPerilaku["TRANSFORMASI DIAGNOSIS PERILAKU SANTRI"]
        HukumGejala["PENGHUKUMAN GEJALA LUAR (LAMA)<br/>• Hanya melihat bentuk fisik pelanggaran (cth: terlambat, gaduh).<br/>• Respon pemukulan & sanksi fisik langsung tanpa diagnosis.<br/>• Masalah berulang terus karena akar penyebab tidak diobati."]
        
        DiagnosisFBA["DIAGNOSIS FBA MODEL ABC (TUMBUH)<br/>• Membedah pemicu (Antecedent) & keuntungan (Consequence).<br/>• Mengidentifikasi fungsi: Menghindar (Escape) vs Mencari (Obtain).<br/>• Merancang BIP & melatih perilaku pengganti (Replacement Behavior)."]
        
        HukumGejala ==>|TRANSFORMASI SAINTIFIK DIAGNOSTIK| DiagnosisFBA
    end
```
...
```mermaid
graph TD
    A["Gugatan Penghakiman Kulit Luar (Pihak A):<br/>'Kita tidak perlu peduli alasan santri berbuat salah; yang penting dia melanggar aturan ya harus dihukum!'"]
    --> B1["Tinjauan Hadits Shahih Bukhari 1:<br/>'Innamal a'malu bin-niyyat' (Setiap amal perbuatan manusia digerakkan oleh niat motif batinnya)."]
    A --> B2["Tinjauan Kaidah Fiqhiyyah As-Suyuthi:<br/>'Al-Umuru bi Maqashidiha' (Status hukum suatu perkara ditentukan oleh maksud & tujuan di baliknya)."]
    A --> B3["Tinjauan Ighatsat al-Lahfan Imam Ibnu Qayyim Al-Jauziyyah:<br/>Kewajiban tabib jiwa mendiagnosis akar syahwat & syubhat sebelum memberikan obat penawar."]
    B1 & B2 & B3 --> C["Titik Temu Konsensus:<br/>Mendiagnosis motif & fungsi perilaku adalah kewajiban syar'i sebelum merumuskan intervensi adab."]
```
...
```mermaid
flowchart LR
    subgraph ModelTigaSukuABC["MODEL TIGA SUKU PERILAKU ABC (O'NEILL ET AL., 1997)"]
        A["1. ANTECEDENT (Pemicu/Konteks)<br/>Kondisi lingkungan sebelum perilaku terjadi:<br/>• Waktu (misal: jam 21.00 WIB).<br/>• Lokasi (misal: kamar mandi asrama).<br/>• Orang (misal: musyrif yang galak).<br/>• Tugas (misal: hafalan matan sulit)."]
        
        B["2. BEHAVIOR (Perilaku Teramati)<br/>Tindakan spesifik santri:<br/>• Berteriak melawak di kamar.<br/>• Berkelahi fisik.<br/>• Menyelinap keluar pondok."]
        
        C["3. CONSEQUENCE (Hasil/Keuntungan)<br/>Apa yang terjadi segera setelahnya:<br/>• Ditertawakan teman (Perhatian).<br/>• Dikeluarkan dari kelas (Lolos tugas).<br/>• Diberikan teguran (Sensasi)."]
...
```mermaid
graph TD
    subgraph DuaFungsiPerilakuSantri["DUA FUNGSI UTAMA PERILAKU SANTRI (FBA ANALYSIS)"]
        F1["1. FUNGSI MENGHINDAR / MELARIKAN DIRI (ESCAPE / AVOID)<br/>• Menghindari tugas akademik yang terlalu sulit (Nahwu/Sains).<br/>• Menghindari interaksi sosial yang mencemaskan / bullying.<br/>• Menghindari kelelahan fisik / rasa malu di depan umum."]
        
        F2["2. FUNGSI MENCARI / MENDAPATKAN (OBTAIN / ACCESS)<br/>• Mencari perhatian & penerimaan dari teman sebaya (Peer Status).<br/>• Mencari perhatian dari musyrif / asatidz.<br/>• Mencari akses benda (makanan, gawai, waktu tidur tambahan)."]
...
```mermaid
graph TD
    subgraph TigaPilarStrategiBIP["3 PILAR RENCANA AKSI PERILAKU (BIP FRAMEWORK)"]
        P1["PILAR 1: MODIFIKASI ANTECEDENT (PENCEGAHAN PEMICU)<br/>Mengubah lingkungan agar perilaku salah tidak terpancing:<br/>• Memecah tugas hafalan menjadi segmen kecil.<br/>• Memindahkan posisi tempat tidur kamar menjauhi pemicu konflik."]
        
        P2["PILAR 2: PENGAJARAN PERILAKU PENGGANTI (REPLACEMENT BEHAVIOR)<br/>Mengajarkan keterampilan baru yang memenuhi fungsi yang sama secara beradab:<br/>• Mengangkat kartu izin istirahat 3 menit saat mulai cemas.<br/>• Mengungkapkan keluhan dengan bahasa santun."]
        
        P3["PILAR 3: PENGUATAN KONSEKUENSI POSITIF (REINFORCEMENT)<br/>Memberikan apresiasi cepat saat santri menggunakan perilaku pengganti:<br/>• Poin bintang logbook PBIS & pujian privat musyrif."]
...
```mermaid
flowchart TD
    FBA1["1. RUJUKAN SANTRI DARI DATA PBIS TIER 2/3 (TRIGGER)<br/>Santri menunjukkan pola pelanggaran berulang $\ge 5$ kali dalam 1 bulan."]
    
    FBA2["2. PENGUMPULAN DATA ABC LAPANGAN (1 PEKAN OBSERVASI)<br/>Musyrif & Guru mencatat lembar observasi ABC di asrama dan kelas madrasah."]
    
    FBA3["3. WAWANCARA FBA TERSTRUKTUR BERSAMA SANTRI (CLINICAL INTERVIEW)<br/>Konselor BK menggali persepsi santri, pemicu stres, & kebutuhan emosinya."]
    
    FBA4["4. SIDANG PERUMUSAN HIPOTESIS FUNGSI & PENYUSUNAN DOKUMEN BIP<br/>Tim PBIS menyepakati rencana intervensi terpadu & menyosialisasikan ke asatidz/ortu."]
