# P5-08-02: Format Catatan Narasi Perkembangan Kualitatif

## Status Dokumen
* **Status**: 🌟 **A+ (Tervalidasi & Siap Diimplementasikan)**
* **Sub-Domain**: `05 Assessment Framework / 08 Mentor Assessment`
* **Penanggung Jawab Keilmuan**: Dewan Keilmuan TUMBUH (*Pakar Pedagogi Master Guru & Pakar Metodologi Riset*)

---

## 1. Spesifikasi Format Narasi Kualitatif (Qualitative Narrative Review)

Setiap akhir bulan, Musyrif dan Wali Kelas melengkapi catatan narasi kualitatif yang menggambarkan profil pertumbuhan santri secara utuh:

```mermaid
graph TD
    NarrativeFormat["3 Bagian Utama Narasi Perkembangan Kualitatif"]
    NarrativeFormat --> Part1["1. Keunggulan Karakter & Kebaikan Menonjol<br/>(Deskripsi perilaku adab & inisiatif positif yang berhasil dicapai)."]
    NarrativeFormat --> Part2["2. Tantangan & Kebutuhan Bimbingan<br/>(Penjelasan aspek adab/pelajaran yang membutuhkan penguatan lanjutan)."]
    NarrativeFormat --> Part3["3. Rencana Dukungan Bersama Ortus-Pengasuh<br/>(Langkah nyata pendampingan pada bulan berikutnya)."]
```

---

## 2. Contoh Pengisian Narasi Kualitatif

- *"Ananda X menunjukkan kemajuan pesat dalam hal kemandirian sholat Subuh dan kebersihan kamar (Jenjang J2). Ananda juga sangat empatis membantu teman kamarnya yang sempat mengalami homesick. Di bulan depan, kami fokus mendampingi ketelitian muraja'ah hafalan Juz 3."*
