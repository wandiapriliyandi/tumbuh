...
```mermaid
graph TD
    subgraph DualPillar["DUAL-PILLAR CARETAKING TUMBUH 24-JAM"]
        Walas["PILAR 2: WALI KELAS (WALAS)<br/>Fokus: Domain Akademik & Adab Kelas (07.00 - 15.00)"]
        Musyrif["PILAR 1: MUSYRIF ASRAMA<br/>Fokus: Domain Pengasuhan & Kehidupan 24-Jam (15.00 - 07.00)"]
        Handover["SESI HANDOVER RESMI 15-MENIT<br/>Sinkronisasi Catatan Logbook, Presensi, & Kondisi Emosional Santri"]
        Walas <--> Handover
        Handover <--> Musyrif
    end
```
...
```mermaid
graph LR
    Laporan["Laporan Masuk (Anonim/Langsung)"] --> Tim["Komite Perlindungan Santri (Independen)"]
    Tim --> Proteksi["Pengamanan & Dukungan Medis/Psikologis Korban"]
    Tim --> Investigasi["Investigasi Faktual & Tabayyun Berkeadilan"]
    Investigasi --> Tindakan["Sanksi Tegas Pelaku & Pemulihan Komunitas Restoratif"]
```
