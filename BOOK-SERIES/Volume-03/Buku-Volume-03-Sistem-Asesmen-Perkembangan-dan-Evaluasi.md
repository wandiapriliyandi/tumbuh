...
```mermaid
graph LR
    Lama["Sistem Poin Defisit (Klasik)<br/>Hanya mencatat dosa -> Labeling & Stigmatisasi -> Santri menjauh dari musyrif"]
    TUMBUH["Sistem Asesmen Ipsatif TUMBUH<br/>Mencatat pertumbuhan fitrah -> Penguatan perilaku positif -> Motivasi intrinsik tumbuh"]
    Lama -->|TRANSFORMASI PARADIGMA| TUMBUH
```
...
```mermaid
graph TD
    Triangulasi["TRIANGULASI ASESMEN ADAB 360-DERAJAT"]
    Triangulasi --> Self["1. Self-Assessment (Muhasabah)<br/>Refleksi kesadaran qalb harian santri"]
    Triangulasi --> Peer["2. Peer-Assessment (Rekan Kamar)<br/>Evaluasi dinamika gotong royong & empati"]
    Triangulasi --> Musyrif["3. Musyrif Asrama<br/>Observasi 24-jam pola hidup & adab kamar"]
    Triangulasi --> Walas["4. Wali Kelas (Walas)<br/>Observasi kesiapan belajar & fokus KBM"]
```
...
```mermaid
graph LR
    ODR["Pencatatan Insiden Faktual (Logbook Digital)"]
    ODR --> Analitik["Analitik Dashboard PBIS: Waktu, Lokasi, Pola Perilaku"]
    Analitik --> Tiers["Klasifikasi Multi-Tier: 80% Tier 1, 15% Tier 2 (CICO), 5% Tier 3 (FBA/BK)"]
```
