...
```mermaid
flowchart TD
    subgraph TransformasiRubrikAsesmen["REKONSTRUKSI SISTEM RUBRIK ASESMEN AKHLAK"]
        AngkaFiktif["PENILAIAN ANGKA SKALA SUBJEKTIF<br/>• Penilaian berbasis tebakan musyrif di akhir semester.<br/>• Rawan bias kemurahan hati (Leniency Bias).<br/>• Santri tidak paham aspek apa yang harus diperbaiki."]
        
        TUMBUH["RUBRIK 4-LEVEL OTENTIK TUMBUH<br/>• Deskriptor perilaku teramati di setiap tingkatan.<br/>• Asesmen formatif berkala dengan umpan balik dialogis.<br/>• Uji reliabilitas inter-rater Cohen's Kappa & evaluasi ipsatif."]
        
        AngkaFiktif ==>|DIREKONSTRUKSI MENJADI| TUMBUH
    end
```
...
```mermaid
flowchart LR
    subgraph TuratsMizanAkhlaq["PRINSIP KEADILAN PENIMBANGAN AMAL TURATS"]
        AyatMizan["QS. Ar-Rahman: 7-9<br/>وَأَقِيمُوا الْوَزْنَ بِالْقِسْطِ وَلَا تُخْسِرُوا الْمِيزَانَ<br/>(Tegakkan Timbangan Keadilan & Jangan Kurangi Neraca)"] --> Inti["Mizanul Akhlaq:<br/>Instrumen Penilaian Karakter yang Adil, Presisi, & Bebas Zalim"]
        Ghazali["Al-Ghazali (Mizanul 'Amal):<br/>Standarisasi Neraca Keseimbangan Jiwa"] --> Inti
        Syathibi["Asy-Syathibi (Al-Muwafaqat):<br/>Verifikasi Fakta Perilaku (Tahqiqul Manath)"] --> Inti
    end
```
...
```mermaid
flowchart TD
    subgraph SainsPsikometriRubrik["STANDAR PSIKOMETRI RUBRIK MATINUL KHULUQ"]
        IRR["Inter-Rater Reliability (Cohen's Kappa & ICC):<br/>• Kesepakatan penilaian antar-musyrif minimal 85%.<br/>• Kalibrasi persepsi melalui pelatihan berkala."]
        
        GTheory["Generalizability Theory (G-Theory):<br/>• Mengontrol varians penilai, setting waktu, & instrumen.<br/>• Memastikan skor mencerminkan kompetensi riil santri."]
        
        Authentic["Authentic Assessment Design (Wiggins):<br/>• Rubrik analitik kriteria deskriptif 4 level.<br/>• Evaluasi terintegrasi dalam rutinitas asrama 24 jam."]
        
        IRR & GTheory & Authentic ==> RubrikOtentik["RUBRIK MATINUL KHULUQ TERSTANDARISASI"]
    end
```
...
```mermaid
flowchart TD
    subgraph EkosistemAsesmen24Jam["SISTEM ASESMEN PERILAKU DIGITAL TERPADU"]
        Logbook["LOGBOOK DIGITAL MUSYRIF (CICO & PBIS)<br/>• Input observasi harian perilaku adab (Micro-Badges & Notes).<br/>• Dokumentasi intervensi restoratif & restitusi adab."]
        
        Triangulasi["TRIANGULASI DATA ASESMEN 360 DERAJAT:<br/>• Self-Assessment Santri (Muhasabah Yaumiyyah).<br/>• Peer-Assessment Teman Kamar (Ukhuwah Review).<br/>• Evaluasi Guru Madrasah & Musyrif Asrama."]
        
        Dashboard["DASHBOARD ANALITIK PERTUMBUHAN IPSATIF:<br/>• Grafik radar pertumbuhan 4 pilar Matinul Khuluq.<br/>• Laporan trajektori perkembangan longitudinal santri J1–J4."]
        
        Logbook --> Triangulasi --> Dashboard
    end
```
...
```mermaid
flowchart TD
    subgraph ProtokolKalibrasiAsesmen["PROTOKOL KALIBRASI DAN REKONSILIASI PENILAIAN"]
        Sesi1["1. FORUM KALIBRASI MUSYRIF-GURU (MODERATION MEETING)<br/>• Mempertemukan musyrif dan guru untuk menyandingkan bukti observasi faktual."]
        
        Sesi2["2. OBSERVASI SILANG & PEMERIKSAAN LOGBOOK BEHAVIORAL<br/>• Menganalisis catatan logbook CICO dan jurnal refleksi diri Santri M."]
        
        Sesi3["3. DIALOG FORMATIF BERSAMA SANTRI M<br/>• Menyajikan data kontradiksi perilaku secara empatik (Firm & Kind).<br/>• Menyepakati target perbaikan spesifik di kamar asrama."]
        
        Sesi4["4. PENETAPAN SKOR IPSATIF AKHIR TERPADU<br/>• Memberikan skor Level 2 (Developing) dengan kontrak perbaikan adab muamalah kamar."]
...
```mermaid
flowchart TD
    subgraph ArsitekturRubrik4Level["EMPAT TINGKAT KEMATANGAN ADAB MATINUL KHULUQ"]
        L1["LEVEL 1: PERINTISAN (EMERGING / PERLU BIMBINGAN)<br/>• Belum konsisten mempraktikkan adab; membutuhkan instruksi langsung musyrif.<br/>• Masih sering tergoda melanggar aturan saat pengawasan lengah."]
        
        L2["LEVEL 2: PEMBIASAAN (DEVELOPING / REGULASI DIRI AWAL)<br/>• Mampu mempraktikkan adab dasar secara mandiri dalam rutinitas terstruktur.<br/>• Masih memerlukan pengingat sesekali saat menghadapi situasi konflik/tekanan."]
        
        L3["LEVEL 3: PEMANTAPAN (PROFICIENT / KONSISTEN OTONOM)<br/>• Menampilkan integritas moral konsisten di ruang ramai maupun privat.<br/>• Berani menolak tekanan buruk sebaya & membela kebenaran (Upstander)."]
        
        L4["LEVEL 4: KETELADANAN (EXEMPLARY / QUDWAH HASANAH)<br/>• Menjadi inspirator dan teladan hidup akhlak mulia bagi lingkungan pesantren.<br/>• Membimbing santri junior dengan kelembutan kasih sayang & memediasi perselisihan."]
...
Penerapan konsep **P3-06-07: RUBRIK INDIKATOR PERILAKU 4-LEVEL MATINUL KHULUQ** di ekosistem pesantren berbasis sistem TUMBUH memerlukan pemahaman multidimensional yang memadukan khazanah turats syariat dan konsensus sains pendidikan modern:
