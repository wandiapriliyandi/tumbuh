...
```mermaid
flowchart TD
    subgraph TransformasiAsesmenIntelektual["REKONSTRUKSI SISTEM ASESMEN KOGNITIF SANTRI"]
        Tunggal["ASESMEN TUNGGAL GURU MADRASAH (TRADISIONAL)<br/>• Nilai rapor hanya berdasarkan lembar ujian tengah/akhir semester.<br/>• Rawan bias subjektivitas guru & kecurangan menyontek.<br/>• Wawasan membaca asrama & nalar kritis terabaikan."]
        
        TUMBUH["TRIANGULASI INTELEKTUAL 360 DERAJAT (TUMBUH)<br/>• Integrasi 5 Sumber: Guru, Musyrif, Dewan Riset, Santri, Sebaya.<br/>• Early Warning System (EWS) mendeteksi hambatan nalar dini.<br/>• Portofolio riset digital & sidang munaqasyah terstandarisasi."]
        
        Tunggal ==>|DIREKONSTRUKSI MENJADI| TUMBUH
    end
```
...
```mermaid
flowchart LR
    subgraph TuratsTakhrijSanad["METODOLOGI TRIANGULASI TURATS"]
        Khatib["Al-Khatib Al-Baghdadi (Al-Kifayah):<br/>Verifikasi Kredibilitas Ilmu Melalui Kesaksian Multi-Guru & Rekan Sebaya"] --> Inti["Al-Istibtsat wal Mu'ayanah:<br/>Pengujian Keilmuan Santri Melalui Multi-Sudut Pandang"]
        IbnuShalah["Ibnu Ash-Shalah (Muqaddimah):<br/>I'tibar: Membandingkan Riwayat untuk Menguji Ketelitian (Dhabth)"] --> Inti
        Syathibi["Asy-Syathibi (Muwafaqat):<br/>Tawatur Ma'nawi: Kebenaran yang Didukung Bukti Jamak"] --> Inti
    end
```
...
```mermaid
flowchart TD
    subgraph SainsTriangulasiKognitif["SINTESIS MODEL MTMM & G-THEORY"]
        MTMM["Multi-Trait Multi-Method (Campbell & Fiske, 1959):<br/>• Menguji validitas konvergen & diskriminan antar-penilai kognitif."]
        
        GTheory["Generalizability Theory (Brennan, 2001):<br/>• Mengisolasi varians kesalahan pengukuran (Rater Bias, Item Bias, Setting Bias)."]
        
        Dashboard["Fikr-TUMBUH Integrated Analytics:<br/>• Algoritma pembobotan otomatis & deteksi diskrepansi skor."]
        
        MTMM & GTheory ==> Dashboard
    end
```
...
```mermaid
flowchart TD
    subgraph AliranDataIntelektual24Jam["ARSITEKTUR ALIRAN DATA FIKR-TUMBUH"]
        Input1["1. GURU MADRASAH: Nilai Ujian Analisis HOTS, Kuis Kaidah Nahwu, & Tugas Esai."]
        Input2["2. MUSYRIF HALAQAH: Observasi Adab Bahtsul Masail, Logbook Membaca 20 Buku, & Nalar Kritis."]
        Input3["3. DEWAN RISET KTI: Skor Proposal KTI, Uji Similaritas Anti-Plagiasi, & Sidang Munaqasyah."]
        Input4["4. SANTRI MANDIRI: Jurnal Muhasabah Metakognisi & Portofolio Resensi Buku Harian."]
        Input5["5. PEER REVIEW SEBAYA: Ulasan Keadilan Berdebat & Integritas Kerja Kelompok."]
        
        Input1 & Input2 & Input3 & Input4 & Input5 ==> Server["ENGINE ANALITIK INTELEKTUAL (FIKR-TUMBUH)"]
        Server ==> Output1["Rapor Profil Nalar & Literasi Santri"]
        Server ==> Output2["Early Warning System (EWS) Kognitif bagi Pembimbing"]
        Server ==> Output3["Transkrip Portofolio Riset Kelulusan Terakreditasi"]
    end
```
...
```mermaid
flowchart TD
    subgraph ProtokolResolusiJokiTugas["PROTOKOL RESTORASI INTEGRITAS AKADEMIK TIER 3"]
        E1["1. SIDANG KLARIFIKASI RESTORATIF DEWAN PENGAWAS AKADEMIK<br/>• Menghadirkan Santri A & Santri J4 dalam forum mediasi tertutup non-kekerasan.<br/>• Membedah hadits Nabi SAW: 'Man ghasysyana falaisa minna' (Siapa yang curang bukan golongan kami)."]
        
        E2["2. PEMBATALAN SELURUH NILAI TUGAS & KONSEKUENSI LOGIS RESTITUSI<br/>• Nilai tugas Santri A dibatalkan; Santri J4 dicabut hak kepengurusan asramanya."]
        
        E3["3. PENULISAN ULANG ESAI SECARA LIVE DI BAWAH PENGAWASAN (DIRECT SUPERVISION)<br/>• Santri A diwajibkan menulis ulang esai di perpustakaan secara mandiri selama 2 pekan."]
        
        E4["4. MONITORING INTEGRITAS LONGITUDINAL (1 SEMESTER)<br/>• Pemantauan intensif mutaba'ah kejujuran belajar bagi kedua santri hingga lulus evaluasi."]
...
```mermaid
flowchart TD
    subgraph LimaPilarTriangulasiIntelektual["LIMA PILAR TRIANGULASI INTELEKTUAL 360 DERAJAT"]
        P1["PILAR 1: GURU MADRASAH (20% Bobot)<br/>• Nilai Ujian HOTS, Analisis Teks Kitab, Eksperimen Sains, & Keaktifan Kelas."]
        
        P2["PILAR 2: MUSYRIF HALAQAH (25% Bobot)<br/>• Observasi Adab Bahtsul Masail, Logbook Membaca 20 Buku, & Nalar Logika Mantiq."]
        
        P3["PILAR 3: DEWAN RISET KTI (35% Bobot)<br/>• Penilaian Monograf KTI Mandiri, Uji Similaritas Anti-Plagiasi, & Sidang Munaqasyah."]
        
        P4["PILAR 4: REFLEKSI SANTRI (10% Bobot)<br/>• Jurnal Metakognisi Yaumiyyah, Resensi Buku Mandiri, & Evaluasi Diri."]
        
        P5["PILAR 5: ULASAN REKAN SEBAYA (10% Bobot)<br/>• Peer Assessment: Integritas Kelompok, Keadilan Berdebat, & Sikap Tawadhu'."]
        
        P1 & P2 & P3 & P4 & P5 ==> Komposit["SKOR INDEKS MUTSAQQAFUL FIKR TERPADU (IK-MF)"]
    end
```
...
| :--- | :--- | :--- | :--- |
| **Jenjang J1 (Kelas 7)** | • Pemantauan minat baca mandiri & pojok baca kamar.<br/>• Penguasaan peta konsep nahwu-sharaf dasar.<br/>• Verifikasi integritas tugas rumah (anti-menyontek). | • Madrasah: Mingguan.<br/>• Musyrif: Harian.<br/>• Refleksi: Mingguan. | Guru Bahasa Arab & Musyrif Kamar J1. |
| **Jenjang J2 (Kelas 8–9)** | • Evaluasi logika mantiq & deteksi sesat pikir.<br/>• Kualitas penulisan esai ilmiah komparatif (1.500 kata).<br/>• Adab bermedia siber & keterampilan cek fakta. | • Madrasah: Tiap bab.<br/>• Halaqah: 2 pekanan.<br/>• Peer: Semesteran. | Guru Ushul Fiqh & Pembina Literasi Digital. |
| **Jenjang J3 (Kelas 10–11)** | • Bimbingan metodologi penulisan Monograf KTI Mandiri.<br/>• Uji similaritas anti-plagiasi Turnitin ($< 15\%$).<br/>• Keaktifan argumentasi dalam sidang Bahtsul Masail. | • Dewan Riset: Bulanan.<br/>• Musyrif: Pekanan.<br/>• Mandiri: Harian. | Dosen Pembimbing Riset & Dewan Penguji KTI. |
| **Jenjang J4 (Kelas 12)** | • Sidang Munaqasyah Terbuka Monograf KTI.<br/>• Evaluasi artikel publikasi jurnal santri & orasi ilmiah.<br/>• Evaluasi peran mentor riset bagi santri junior J1. | • Komprehensif: 1x menjelang wisuda kelulusan. | Dewan Guru Besar Riset & Pimpinan Pesantren. |
