...
```mermaid
flowchart TD
    subgraph TransformasiInstrumentasiIntelektual["REKONSTRUKSI INSTRUMENTASI EVALUASI KOGNITIF"]
        TanpaInstrumen["EVALUASI SUBJEKTIF TANPA DOKUMEN (TRADISIONAL)<br/>• Tidak ada rekam jejak buku yang dibaca di asrama.<br/>• Bimbingan karya ilmiah tanpa formulir monitoring.<br/>• Nilai rapor hanya berdasarkan kesan subjektif ustadz."]
        
        TUMBUH["PAKET INSTRUMEN MASTER MUTSAQQAFUL FIKR (TUMBUH)<br/>• LOF-MF: Lembar observasi nalar kritis asrama.<br/>• JRM-MF: Jurnal metakognisi & resensi buku harian.<br/>• CBR-MF: Kartu bimbingan KTI & audit Turnitin.<br/>• FPS-MF: Formulir sidang munaqasyah terstandarisasi."]
...
```mermaid
flowchart LR
    subgraph TuratsTadwinIlmu["TRADISI DOKUMENTASI ILMU DALAM TURATS"]
        HaditsIkatIlmu["HR. Ad-Darimi & Ath-Thabrani:<br/>قَيِّدُوا الْعِلْمَ بِالْكِتَابِ<br/>(Ikatlah Ilmu Itu dengan Menuliskannya / Dokumentasi Rapi)"] --> Inti["Hakikat Mutaba'ah Fikr:<br/>Pencatatan Rekam Jejak Literasi & Riset Secara Presisi"]
        IbnuShalah["Ibnu Ash-Shalah (Muqaddimah):<br/>Tradisi Samā' & Qirā'ah Wajib Disertai Lembar Catatan Pengesahan Guru"] --> Inti
        Khatib["Al-Khatib Al-Baghdadi (Taqyidul 'Ilm):<br/>Keagungan Tulisan dalam Menjaga Ketelitian Ilmu"] --> Inti
    end
```
...
```mermaid
flowchart TD
    subgraph SainsPengukuranKognitif["VALIDITAS INSTRUMEN MUTSAQQAFUL FIKR"]
        Construct["Construct Validity (Aiken's V >= 0.92):<br/>• Uji validitas isi oleh pakar kurikulum, ahli mantiq, & dewan riset."]
        
        Logging["Behavioral Logging Protocol:<br/>• Pencatatan data harian terverifikasi mengurangi recall bias."]
        
        Turnitin["Plagiarism Detection Integration:<br/>• Integrasi rekam uji Turnitin ke dalam lembar CBR-MF."]
        
        Construct & Logging & Turnitin ==> InstrumenStandar["PAKET INSTRUMEN MASTER TERSTANDARISASI"]
    end
```
...
```mermaid
flowchart TD
    subgraph AlurMutabaahIntelektual["ALUR MUTABA'AH INTELEKTUAL 24 JAM"]
        SantriIsi["1. SANTRI MENGISI JRM-MF: Mencatat judul buku yang dibaca, resume 3 poin, & refleksi metakognisi harian."]
        MusyrifCek["2. MUSYRIF MEMVALIDASI LOF-MF: Memeriksa pojok baca kamar, menguji pemahaman santri, & paraf mingguan."]
        PembimbingKTI["3. DOSEN PEMBIMBING MENGISI CBR-MF: Mencatat progres bab KTI, revisi metodologi, & skor Turnitin."]
        AdminInput["4. INPUT DATA KE DASHBOARD FIKR-TUMBUH: Seluruh skor disinkronkan ke basis data digital pesantren."]
        
        SantriIsi --> MusyrifCek --> PembimbingKTI --> AdminInput
    end
```
...
```mermaid
flowchart TD
    subgraph ProtokolAuditLiterasi["PROTOKOL RESTORASI KEJUJURAN LITERASI TIER 2"]
        Step1["1. DIALOG RESTORATIF 'MEMBACA DENGAN HATI BUKAN DENGAN ANGKA'<br/>• Musyrif berdialog empatik, membedah hakikat mencari ilmu demi ridha Allah, bukan nilai semu."]
        
        Step2["2. PEMBATALAN CATATAN PALSU & RESET TARGET MEMBACA REALISTIS<br/>• Catatan palsu dibatalkan; target disesuaikan: membaca 1 buku per 2 pekan secara mendalam."]
        
        Step3["3. METODE ORAL BOOK REVIEW (RESENSI LISAN MINGGUAN)<br/>• Santri F diwajibkan menceritakan isi 1 bab buku yang dibacanya setiap malam Ahad kepada musyrif."]
        
        Step4["4. PEMULIHAN PREDIKAT INTEGRITAS LITERASI<br/>• Dalam 6 pekan, Santri F berhasil menuntaskan 3 buku bermutu secara jujur & memahami isinya dengan sangat baik."]
...
```mermaid
flowchart TD
    subgraph EmpatInstrumenMasterIntelektual["EMPAT INSTRUMEN MASTER MUTSAQQAFUL FIKR"]
        LOF["1. FORM LOF-MF (LEMBAR OBSERVASI NALAR KRITIS ASRAMA)<br/>• Target: Musyrif Asrama & Guru Madrasah.<br/>• Fungsi: Menilai keaktifan bertanya HOTS, nalar mantiq, adab Bahtsul Masail, & pojok baca kamar."]
        
        JRM["2. FORM JRM-MF (JURNAL METAKOGNISI & RESENSI BUKU SANTRI)<br/>• Target: Santri Mandiri.<br/>• Fungsi: Mencatat judul buku harian, 3 poin gagasan utama, refleksi belajar, & target KTI."]
        
        CBR["3. FORM CBR-MF (KARTU BIMBINGAN RISET & AUDIT KTI)<br/>• Target: Dosen Pembimbing Riset KTI.<br/>• Fungsi: Merekam progres 5 bab KTI, catatan revisi metodologi, & skor uji Turnitin."]
        
        FPS["4. FORM FPS-MF (FORMULIR PENILAIAN SIDANG MUNAQASYAH)<br/>• Target: Dewan Penguji Sidang Terbuka.<br/>• Fungsi: Menguji penguasaan Turats, orisinalitas riset, metodologi, & retorika presentasi."]
