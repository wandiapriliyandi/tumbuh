...
```mermaid
flowchart TD
    subgraph RekonstruksiTaksonomiIntelektual["REKONSTRUKSI TAKSONOMI KOMPETENSI INTELEKTUAL"]
        Rendah["TAKSONOMI HAFALAN RENDAH (TRADISIONAL)<br/>• 80% soal ujian sekadar mengingat definisi teks (C1-C2).<br/>• Tanpa kemampuan analisis dalil, logika mantiq, & riset.<br/>• Santri pasif & tidak mandiri dalam memecahkan masalah."]
        
        TUMBUH["TAKSONOMI NALAR FITRAH TUMBUH (HOTS)<br/>• Integrasi 4 Level: Ta'allum -> Tahlil -> Istinbath -> Ijtihad/Qudwah.<br/>• Penguasaan Mantiq, Maqashid Syari'ah, Riset KTI, & Digital.<br/>• Standar SKI-SKD J1–J4 melahirkan Intelektual Kosmopolitan."]
        
        Rendah ==>|DIREKONSTRUKSI MENJADI| TUMBUH
    end
```
...
```mermaid
flowchart LR
    subgraph TuratsMaratibulIdrak["TINGKATAN PERSEPSI KOGNITIF DALAM TURATS"]
        Idrak["Maratibul Idrak Klasik:<br/>Jahl Basith/Murakkab -> Wahm -> Syakk -> Zhann -> 'Ilm / Yaqin"] --> Inti["Taksonomi Nalar Fitrah:<br/>Dari Pengetahuan Dasar Menuju Keyakinan Ilmiah Demonstratif (Yaqin Burhani)"]
        Amidi["Al-Amidi (Al-Ihkam):<br/>Tingkatan Istinbath: Fahmul Khithab -> Dalalatun Nash -> Qiyas"] --> Inti
        Syathibi["Asy-Syathibi (Muwafaqat):<br/>Mencapai Derajat Fahmu Maqashidis Syari'ah"] --> Inti
    end
```
...
```mermaid
flowchart TD
    subgraph KonvergensiTaksonomiKognitif["SINTESIS TAKSONOMI KOGNITIF MULTIDIMENSI"]
        Anderson["Revisi Taksonomi Bloom (Anderson & Krathwohl, 2001):<br/>Remember -> Understand -> Apply -> Analyze -> Evaluate -> Create"]
        
        Marzano["Marzano's New Taxonomy (2007):<br/>Retrieval -> Comprehension -> Analysis -> Knowledge Utilization -> Metacognitive System -> Self-System"]
        
        Fitrah["Taksonomi Nalar Fitrah TUMBUH:<br/>1. Ta'allum wa Fahm (Literasi & Konsep Dasar)<br/>2. Tahlil wa Istidlal (Analisis Dalil & Logika Mantiq)<br/>3. Istinbath wa Bahts (Riset KTI & Fiqh Kontekstual)<br/>4. Ijtihad wa Qudwah Fikr (Sintesis Hikmah & Kepemimpinan)"]
        
        Anderson & Marzano ==> Fitrah
    end
```
...
```mermaid
flowchart TD
    subgraph KurikulumIntelektual24Jam["INTEGRASI KURIKULUM INTELEKTUAL ASRAMA 24 JAM"]
        Madrasah["JAM MADRASAH: Pembelajaran HOTS, Kaidah Ushul Fiqh, Logika Mantiq, & Sains."]
        Maktabah["JAM MAKTABAH: Riset Digital Maktabah Syamilah, Penelusuran Jurnal, & Penulisan KTI."]
        Asrama["JAM ASRAMA: Bahtsul Masail Dialogis, Pojok Baca Senyap, & Debat Ilmiah Beradab."]
        
        Madrasah <==> Maktabah <==> Asrama
    end
```
...
```mermaid
flowchart TD
    subgraph ProtokolScaffoldingAbstrak["PROTOKOL PENDAMPINGAN NALAR ABSTRAK TIER 2"]
        StepA["1. PENGGUNAAN DIAGRAM ANALOGI VISUAL (VISUAL QIYAS MATRIX)<br/>• Ustadz menyajikan bagan 4 rukun Qiyas: Asal (Khamr) -> Far' (Narkoba) -> 'Illat (Memabukkan) -> Hukum (Haram)."]
        
        StepB["2. STUDI KASUS KONKRET DUNIA REMAJA (CASE-BASED LEARNING)<br/>• Santri L diajak menganalisis kasus konkret yang dekat dengan kehidupannya."]
        
        StepC["3. DIALOG SOCRATIC KELOMPOK KECIL (PEER COLLABORATION)<br/>• Santri L berdiskusi dengan 2 teman sebaya untuk menguji hipotesis 'illat hukum."]
        
        StepD["4. EVALUASI FORMATIF PEMAHAMAN MANDIRI (SELF-EXPLANATION)<br/>• Santri L menjelaskan ulang konsep Qiyas menggunakan bahasanya sendiri."]
...
```mermaid
flowchart TD
    subgraph HierarkiTaksonomiNalarFitrah["EMPAT LEVEL TAKSONOMI NALAR FITRAH MUTSAQQAFUL FIKR"]
        L4["LEVEL 4: IJTIHAD WA QUDWAH FIKR (SINTESIS HIKMAH & KEPEMIMPINAN INTELEKTUAL)<br/>• Mampu mensintesiskan Turats, Sains, & Filsafat untuk solusi krisis peradaban.<br/>• Menjadi rujukan ilmiah, mentor riset junior, & juru bicara kemaslahatan umat."]
        
        L3["LEVEL 3: ISTINBATH WA BAHTS (RISET ILMIAH, KTI MANDIRI, & FIQH KONTEKSTUAL)<br/>• Menyelesaikan penulisan Monograf Karya Tulis Ilmiah (KTI) metodologis $\ge 20\text{ halaman}$.<br/>• Menguasai analisis Maqashid Syari'ah & literasi riset digital terstandarisasi."]
        
        L2["LEVEL 2: TAHLIL WA ISTIDLAL (ANALISIS DALIL, LOGIKA MANTIQ, & DETEKSI FALLACY)<br/>• Mampu mendeteksi 10 sesat pikir utama & menganalisis kekuatan dalil komparatif.<br/>• Aktif dalam debat ilmiah terstruktur & menulis esai argumentatif 1.500 kata."]
        
        L1["LEVEL 1: TA'ALLUM WA FAHM (LITERASI DASAR & PENGUASAAN KONSEP TEKS)<br/>• Menguasai kaidah dasar nahwu-sharaf & peta konsep matan kitab fikih dasar.<br/>• Membaca mandiri 20 buku/tahun & membedakan fakta vs opini dengan prinsip tabayyun."]
...
| :--- | :--- | :--- |
| **Pilar 1: Ma'rifah Turatsiyyah** | **SKD-MF-1.1**<br/>**SKD-MF-1.2**<br/>**SKD-MF-1.3** | Menguasai gramatika bahasa Arab (Nahwu-Sharaf) dan membaca kitab kuning berharakat.<br/>Memahami metodologi Ushul Fiqh, kaidah istinbath, dan Maqashid Syari'ah.<br/>Menganalisis perbedaan dalil mazhab fikih secara komparatif (*Bidayatul Mujtahid*). |
| **Pilar 2: Tafakkur Naqdiy** | **SKD-MF-2.1**<br/>**SKD-MF-2.2**<br/>**SKD-MF-2.3** | Menerapkan kaidah logika silogisme mantiq dan mengidentifikasi sesat pikir (*Logical Fallacy*).<br/>Menerapkan keterampilan berpikir kritis HOTS dalam membedah isu sosial kontemporer.<br/>Mengembangkan kesadaran metakognisi dalam merancang strategi belajar mandiri. |
| **Pilar 3: Literasi Sains & Riset** | **SKD-MF-3.1**<br/>**SKD-MF-3.2**<br/>**SKD-MF-3.3** | Memahami metode ilmiah empiris, hukum alam (*Sunnatullah*), dan analisis data statistik.<br/>Menyusun proposal dan laporan Karya Tulis Ilmiah (KTI) terstandarisasi dengan sitasi APA 7th.<br/>Mempresentasikan hasil riset ilmiah dalam forum sidang munaqasyah terbuka. |
| **Pilar 4: Literasi Digital** | **SKD-MF-4.1**<br/>**SKD-MF-4.2**<br/>**SKD-MF-4.3** | Menerapkan prinsip tabayyun dan verifikasi fakta (Cek Fakta) terhadap informasi digital.<br/>Mengoperasikan perangkat lunak riset (*Reference Manager* Mendeley, Maktabah Syamilah).<br/>Memproduksi konten dakwah digital berbasis riset yang mencerahkan dan beradab. |
