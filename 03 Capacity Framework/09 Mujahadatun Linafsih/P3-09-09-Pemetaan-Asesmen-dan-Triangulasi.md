...
```mermaid
flowchart TD
    subgraph TransformasiAsesmenJiwa["REKONSTRUKSI SISTEM ASESMEN KEMATANGAN JIWA"]
        Tunggal["PENILAIAN TUNGGAL BUKU PELANGGARAN (TRADISIONAL)<br/>• Hanya mencatat santri yang melanggar aturan.<br/>• Korban bullying relasional luput dari pengawasan.<br/>• Guru kelas & musyrif asrama bekerja tanpa integrasi data."]
        
        TUMBUH["TRIANGULASI KEMATANGAN JIWA 360 DERAJAT (TUMBUH)<br/>• Integrasi 5 Sumber: Musyrif, Guru, Santri, Sebaya, Konselor BK.<br/>• Early Warning System (EWS) mendeteksi depresi & bullying dini.<br/>• Dashboard Sakinah-TUMBUH memantau kesejahteraan santri 24 jam."]
...
```mermaid
flowchart LR
    subgraph TuratsUjiAkhlak["METODOLOGI PENGUJIAN AKHLAK SALAF"]
        UmarBinKhattab["Kaidah Sayyidina Umar RA:<br/>Apakah Engkau Pernah Bertetangga Dengannya, Bersafar Bersamanya, & Bermuamalah Dinar-Dirham Dengannya?"] --> Inti["Asy-Syuhud wal Mu'ayanah:<br/>Pengujian Karakter Melalui Observasi Multi-Situasi"]
        Ghazali["Al-Ghazali (Ihya'):<br/>Akhlak Sejati Hanya Terbukti Saat Nafsu Diuji Kemarahan & Kesulitan"] --> Inti
        Muhasibi["Al-Muhasibi (Ar-Ri'ayah):<br/>Verifikasi Kesucian Niat Melalui Kejujuran Pengakuan Diri"] --> Inti
    end
```
...
```mermaid
flowchart TD
    subgraph SainsSosiometriAfektif["SINTESIS MULTI-INFORMANT & SOSIOMETRI"]
        MultiInformant["Multi-Informant Assessment (Achenbach, 2006):<br/>• Menggabungkan persepsi otoritas (musyrif/guru), self-report (santri), & peer report."]
        
        Sociometry["Sociometric Matrix (Jacob Moreno):<br/>• Memetakan struktur penerimaan sosial kamar asrama & mendeteksi santri terisolasi (Isolates)."]
        
        Dashboard["Sakinah-TUMBUH Analytics:<br/>• Algoritma EWS otomatis memicu alarm saat terjadi diskrepansi data tajam."]
        
        MultiInformant & Sociometry ==> Dashboard
    end
```
...
```mermaid
flowchart TD
    subgraph AliranDataJiwa24Jam["ARSITEKTUR ALIRAN DATA SAKINAH-TUMBUH"]
        Input1["1. MUSYRIF ASRAMA: Observasi Pengendalian Amarah, Antre Makan, & Adab Tidur."]
        Input2["2. GURU MADRASAH: Observasi Kesabaran Belajar, Sportivitas, & Kejujuran Kelas."]
        Input3["3. SANTRI MANDIRI: Jurnal Muhasabah Batin, Rekam Tahajjud, & Puasa Sunnah."]
        Input4["4. SOSIOMETRI SEBAYA: Peer Assessment: Keharmonisan Kamar & Ketiadaan Bullying."]
        Input5["5. KONSELOR BK: Catatan Sesi Bimbingan Regulasi Emosi & Konseling Krisis."]
        
        Input1 & Input2 & Input3 & Input4 & Input5 ==> Server["ENGINE ANALITIK KESEJAHTERAAN SANTRI (SAKINAH-TUMBUH)"]
        Server ==> Output1["Rapor Profil Kematangan Jiwa Santri"]
        Server ==> Output2["Early Warning System (EWS) Krisis Emosi bagi Musyrif"]
        Server ==> Output3["Sosiogram Iklim Ukhuwah Kamar Asrama"]
    end
```
...
```mermaid
flowchart TD
    subgraph ProtokolResolusiPengucilan["PROTOKOL RESTORASI SOSIAL UKHUWAH TIER 2"]
        StepA["1. SESI LINGKARAN DIALOG KAMAR (PEER RESTORATIVE CIRCLE)<br/>• Musyrif memfasilitasi dialog empati membedah hadits: 'Perumpamaan kaum mukmin laksana satu tubuh'."]
        
        StepB["2. DEKONSTRUKSI STIGMATISASI SUKU / LOGAT BAHASA<br/>• Musyrif memaparkan keindahan keragaman budaya umat Islam dalam khazanah sahabat Nabi."]
        
        StepC["3. PROYEK KOLABORASI KELOMPOK KAMAR TERPANDU<br/>• Santri Y dipasangkan dengan santri paling berpengaruh di kamar untuk memimpin kebersihan kamar."]
        
        StepD["4. AUDIT SOSIOMETRI ULANG (30 HARI)<br/>• Skor penerimaan Santri Y melonjak ke kategori Populer & keharmonisan kamar pulih 100%."]
...
```mermaid
flowchart TD
    subgraph LimaPilarTriangulasiJiwa["LIMA PILAR TRIANGULASI KEMATANGAN JIWA 360 DERAJAT"]
        P1["PILAR 1: MUSYRIF ASRAMA (35% Bobot)<br/>• Observasi Regulasi Amarah, Antre Tertib, Shalat Subuh, & Adab Tidur 24 Jam."]
        
        P2["PILAR 2: GURU MADRASAH (20% Bobot)<br/>• Observasi Kesabaran Belajar, Sportivitas Olahraga, & Kejujuran Ujian Kelas."]
        
        P3["PILAR 3: SANTRI MANDIRI (20% Bobot)<br/>• Jurnal Muhasabah Batin Harian, Rekam Puasa Sunnah, & Shalat Malam."]
        
        P4["PILAR 4: ULASAN SEBAYA (15% Bobot)<br/>• Peer Sociometric Review: Keharmonisan Kamar & Ketiadaan Bullying."]
        
        P5["PILAR 5: KONSELOR BK (10% Bobot)<br/>• Catatan Bimbingan Regulasi Emosi, Tes DERS, & Resolusi Konflik."]
        
        P1 & P2 & P3 & P4 & P5 ==> Komposit["SKOR INDEKS MUJAHADATUN LINAFSIH TERPADU (IK-ML)"]
    end
```
...
| :--- | :--- | :--- | :--- |
| **Jenjang J1 (Kelas 7)** | • Adaptasi homesickness & keterikatan kamar.<br/>• Kepatuhan bangun subuh & antre makan.<br/>• Deteksi dini intimidasi santri senior. | • Musyrif: Harian.<br/>• Guru: Mingguan.<br/>• Sosiometri: Bulanan. | Musyrif Kamar J1 & Konselor BK Remaja. |
| **Jenjang J2 (Kelas 8–9)** | • Regulasi amarah saat berselisih sebaya.<br/>• Pemantauan adab ghadhdhul bashar siber.<br/>• Keharmonisan dinamika kelompok sekamar. | • Musyrif: Pekanan.<br/>• Peer: 2 Bulanan.<br/>• BK: Sesuai Kasus. | Musyrif Asrama & Pembina Beladiri Santri. |
| **Jenjang J3 (Kelas 10–11)** | • Kemandirian tahajjud & puasa sunnah.<br/>• Resiliensi mental hafalan & ujian.<br/>• Kebersihan hati dari riya' dan dengki. | • Mandiri: Harian.<br/>• Musyrif: 2 Pekanan.<br/>• Guru: Bulanan. | Dosen Pembimbing Halaqah & Musyrif J3. |
| **Jenjang J4 (Kelas 12)** | • Keteladanan akhlak pengayom adik kelas.<br/>• 0% keterlibatan perpeloncoan senioritas.<br/>• Kestabilan jiwa menghadapi kelulusan. | • Komprehensif: 1x per semester menjelang wisuda. | Dewan Pengawas Kepengasuhan & Pimpinan Pondok. |
