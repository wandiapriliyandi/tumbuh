...
```mermaid
flowchart TD
    subgraph TransformasiMetodologiTriangulasi["TRANSFORMASI METODOLOGI ASESMEN KARAKTER PESANTREN"]
        SingleSource["ASESMEN SUMBER TUNGGAL (KONVENSIONAL)<br/>• Bergantung pada opini sepihak satu guru/musyrif.<br/>• Buta terhadap perilaku santri di lokus lain.<br/>• Rawan manipulasi kesan & diskriminasi rapor."]
        
        TriangulasiTUMBUH["TRIANGULASI DATA 360° (TUMBUH PBIS)<br/>• 4 Sumber: Musyrif + Guru + Self-Muhasabah + Peer.<br/>• Validasi silang lintas 4 lokus asrama 24 jam.<br/>• Menghasilkan profil pertumbuhan karakter yang otentik."]
        
        SingleSource ==>|DIREKONSTRUKSI MENJADI| TriangulasiTUMBUH
    end
```
...
```mermaid
graph TD
    A["Gugatan Sumber Tunggal (Pihak A):<br/>'Cukup musyrif kamar saja yang menilai santri; tidak perlu repot-repot mengumpulkan data dari guru kelas atau teman sekamar!'"]
    --> B1["Tinjauan Tafsir QS. Yasin: 65 & QS. An-Nur: 24:<br/>Pengadilan akhirat menegakkan triangulasi persaksian mutlak: Mulut dikunci, tangan berbicara, & kaki bersaksi atas apa yang dikerjakan."]
    A --> B2["Tinjauan Fiqh Tazkiyah asy-Syuhud (Al-Mawardi):<br/>Kredibilitas seseorang diuji melalui kesaksian tetangga dekat & rekan muamalah sehari-hari."]
    A --> B3["Tinjauan Kitab Ihya' 'Ulumiddin (Al-Ghazali):<br/>Muhasabatun Nafsi (Introspeksi Diri) adalah rukun penyucian jiwa sebelum hisab eksternal."]
    B1 & B2 & B3 --> C["Titik Temu Konsensus:<br/>Triangulasi 360° adalah manifestasi keadilan epistemologis syariat Islam dalam menilai integritas insan."]
```
...
```mermaid
graph TD
    A["Gugatan Kerumitan Riset (Pihak A):<br/>'Teori triangulasi Norman Denzin dan MTMM Campbell-Fiske itu terlalu akademis untuk dipraktikkan di pondok pesantren harian!'"]
    --> B1["Tinjauan Teori Triangulasi Norman Denzin (1978):<br/>Menggabungkan Data Triangulation (waktu, ruang, orang) & Methodological Triangulation (observasi, refleksi, tes) melipatgandakan validitas konstruk."]
    A --> B2["Tinjauan MTMM Campbell & Fiske (1959):<br/>Membedakan Convergent Validity (kesepakatan multi-metode pada sifat yang sama) & Discriminant Validity (memisahkan karakter aqidah dari kepandaian bicara)."]
    A --> B3["Tinjauan Desain Sistem Informasi PBIS Terotomasi:<br/>Komputasi digital menghitung rumus agregasi triangulasi secara instan tanpa membebani musyrif."]
    B1 & B2 & B3 --> C["Titik Temu Konsensus:<br/>Triangulasi multimoda modern memberikan jaminan validitas ilmiah tertinggi bagi rapor karakter pesantren."]
```
...
```mermaid
flowchart LR
    subgraph KonvergensiTriangulasiTUMBUH["MATRIKS TRIANGULASI 360° SALIMUL AQIDAH"]
        Musyrif["1. OBSERVASI IN-VIVO MUSYRIF<br/>(Lokus Privat & Masjid - Bobot 40%)"]
        Guru["2. PENGAMATAN GURU KELAS<br/>(Lokus Akademik - Bobot 30%)"]
        Self["3. JURNAL SELF-MUHASABAH<br/>(Refleksi Batin Santri - Bobot 20%)"]
        Peer["4. KONFIRMASI TEMAN KAMAR<br/>(Sosiometri Ukhuwah - Bobot 10%)"]
        
        Musyrif & Guru & Self & Peer ==> AgregasiPBIS["MESIN ANALITIK TRIANGULASI DATA PBIS 24 JAM"]
        AgregasiPBIS ==> ProfilOtentik["PROFIL OTENTIK SALIMUL AQIDAH SANTRI (J1–J4)"]
    end
```
...
```mermaid
flowchart TD
    subgraph ArsitekturAsesmen360TUMBUH["ARSITEKTUR EMPAT SUMBER DATA ASESMEN 360°"]
        
        subgraph Sumber1["1. OBSERVASI IN-VIVO MUSYRIF (40%)"]
            S1_A["• Kepatuhan shalat berjamaah & dzikir ma'tsurat."]
            S1_B["• Integritas ruang privat (Zero ghashab sandal/pakaian)."]
            S1_C["• Kerapian lemari & adab tidur sunnah 22.00."]
        end
        
        subgraph Sumber2["2. PENGAMATAN GURU KELAS (30%)"]
            S2_A["• Kejujuran mutlak saat ujian & pengerjaan tugas mandiri."]
            S2_B["• Adab thalabul ilmi & ketundukan nalar pada syariat."]
            S2_C["• Sikap kritis menelaah ayat kauniyah & sains."]
        end
        
        subgraph Sumber3["3. JURNAL SELF-MUHASABAH (20%)"]
            S3_A["• Refleksi batin mingguan tentang keikhlasan niat."]
            S3_B["• Pengakuan jujur kekurangan diri & tekad taubat."]
            S3_C["• Evaluasi rasa pengawasan Allah (Muraqabatullah)."]
        end
        
        subgraph Sumber4["4. KONFIRMASI TEMAN KAMAR (10%)"]
            S4_A["• Sosiometri Ukhuwah: Penghormatan hak milik sesama."]
            S4_B["• Konfirmasi sikap santun & saling mengingatkan doa."]
            S4_C["• Nihil intimidasi & ketiadaan perilaku merugikan."]
        end
    end
```
...
```mermaid
flowchart TD
    subgraph AlurRekonsiliasiDiskrepansiTUMBUH["PROTOKOL REKONSILIASI DISKREPANSI TRIANGULASI 360°"]
        GapData["1. GAP ALERT: Diskrepansi Ekstrem Nilai Guru (Level 4) vs Musyrif (Level 1)"]
        
        SidangPleno["2. SIDANG KONSULTASI TRIANGULASI (Guru, Musyrif, Konselor BK)<br/>• Menelaah bukti logbook asrama, jurnal muhasabah, & sosiometri kamar.<br/>• Mengidentifikasi akar masalah psikologis di balik perilaku santri."]
        
        DiagnosisKarakter["3. DIAGNOSIS HOLISTIK (Akar Masalah: Stres Ekspektasi Akademik)<br/>• Menolak kompromi rata-rata buta; menetapkan intervensi tepat sasaran."]
        
        IntervensiTier2["4. PROGRAM RESTORASI & KONSELING TIER 2 PBIS (30 Hari)<br/>• Bimbingan regulasi emosi bersama BK & proyek ukhuwah kamar."]
        
        VerifikasiOtentik["5. VERIFIKASI CAPAIAN PARIPURNA (Integrasi Kesalehan Kelas & Kamar)"]
        
        GapData --> SidangPleno --> DiagnosisKarakter --> IntervensiTier2 --> VerifikasiOtentik
    end
```
...
```mermaid
flowchart TD
    subgraph ProtokolRekonsiliasiPBIS["PROTOKOL REKONSILIASI DISKREPANSI DATA LINTAS LOKUS"]
        Deteksi["1. DETEKSI OTOMATIS: Sistem PBIS mendeteksi selisih skor antar-lokus > 1.5 level."]
        
        Notifikasi["2. NOTIFIKASI KASUS: Sistem mengirim undangan Sesi Rekonsiliasi ke Wali Kamar & Wali Kelas."]
        
        Musyawarah["3. MUSYAWARAH TRIANGULASI (15 Menit): Membedah bukti konkret logbook, jurnal, & sosiometri."]
        
        Konsensus["4. PENETAPAN SKOR KONSENSUS: Menetapkan skor berjangkar pada bukti ruang privat terendah."]
        
        Tindakan["5. RENCANA AKSI RESTORATIF: Menetapkan intervensi pembinaan terpadu kelas-asrama (Tier 1/2)."]
        
        Deteksi --> Notifikasi --> Musyawarah --> Konsensus --> Tindakan
    end
```
