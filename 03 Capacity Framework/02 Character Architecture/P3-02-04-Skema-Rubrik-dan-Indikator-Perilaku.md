...
```mermaid
graph TD
    A["Gugatan Intuisionisme (Pihak A):<br/>'Musyrif senior punya firasat batin menilai santri; tidak butuh rubrik BARS dan deskriptor perilaku yang bertele-tele!'"]
    --> B1["Tinjauan Hadits Nabawi Kaidah Peradilan:<br/>'Al-Bayyinatu 'alal mudda'i' (Bukti autentik wajib dihadirkan oleh penuduh/penilai - HR. Al-Bukhari)."]
    A --> B2["Tinjauan Kaidah Ushul Fiqh:<br/>'Al-Yaqiinu la yazaalu bisy-syakk' (Keyakinan hukum tidak boleh digugurkan oleh prasangka/asumsi subjektif semata)."]
    A --> B3["Tinjauan Kaidah Keadilan Islam:<br/>Menilai santri secara tidak adil atau memfitnah perilakunya adalah dosa kezaliman (Zhulm)."]
    B1 & B2 & B3 --> C["Titik Temu Konsensus:<br/>Rubrik BARS adalah instrumen penegakan Bayyinah dan keadilan syariat dalam menilai santri."]
```
...
flowchart TD
    subgraph Mitigasi4BiasPsikometrik["STRATEGI MITIGASI 4 BIAS PENILAI MUSYRIF"]
        ...
        Contrast["4. CONTRAST EFFECT BIAS<br/>(Santri dinilai dibanding teman sekamar, bukan standar)<br/>-> Mitigasi: Rubrik kriteria mutlak (Criterion-Referenced)."]
        
        Halo & Central & Leniency & Contrast ==> Presisi["PENILAIAN AKURAT, ADIL, & MEMILIKI VALIDITAS TINGGI"]
    end
```
...
```mermaid
flowchart TD
    subgraph AlurPencatatanCepatPBIS["PROTOKOL FAST-LOGGING PBIS MUSYRIF"]
        Observasi["1. PENGAMATAN SPONTAN: Musyrif melihat santri melakukan perilaku konkret (cth: membersihkan tempat wudhu)."]
        
        BukaAplikasi["2. BUKA APLIKASI DIGITAL (<5 Detik): Memilih nama santri / tap barcode kamar."]
        
        PilihJangkar["3. TAP JANGKAR PERILAKU (<10 Detik): Memilih ikon 'Nafi'un Lighairih - T3 (Membersihkan Fasilitas Bersama)'. (+ Poin Apresiasi)"]
        
        Selesai["4. DATA TERSIMPAN REALTIME (<5 Detik): Logbook terupdate; santri menerima notifikasi penguatan positif di dashboard."]
        
        Observasi --> BukaAplikasi --> PilihJangkar --> Selesai
    end
```
