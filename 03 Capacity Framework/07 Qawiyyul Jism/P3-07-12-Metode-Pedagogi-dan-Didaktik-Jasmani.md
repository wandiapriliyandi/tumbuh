...
```mermaid
flowchart TD
    subgraph TransformasiDidaktikJasmani["REKONSTRUKSI METODE DIDAKTIK KESEHATAN"]
        Otoriter["METODE KOMANDO & MARAH-MARAH (TRADISIONAL)<br/>• Olahraga militeristik kaku tanpa penjelasan fisiologis.<br/>• Edukasi sanitasi mengandalkan ancaman razia & teror.<br/>• Santri trauma, pasif, & kebersihan runtuh saat tidak diawasi."]
        
        TUMBUH["DIDAKTIK QAWIYYUL JISM TUMBUH (HOLISTIK)<br/>• Fitts & Posner 3 Tahap Penguasaan Keterampilan Motorik.<br/>• Nudge Theory: Penataan visual & pilihan ramah kebersihan.<br/>• Keteladanan hidup sehat guru-musyrif & olahraga sunnah gembira."]
        
        Otoriter ==>|DIREKONSTRUKSI MENJADI| TUMBUH
    end
```
...
```mermaid
flowchart LR
    subgraph TuratsPedagogiJasmani["METODE DIDAKTIK KESEHATAN NABAWI"]
        HaditsWudhu["HR. Al-Bukhari No. 159:<br/>Demonstrasi Praktik Wudhu Sempurna oleh Utsman bin Affan RA<br/>Mencontoh Persis Cara Wudhu Rasulullah SAW"] --> Inti["Didaktik Thibb Nabawi:<br/>Keteladanan Visual (Modeling), Pembiasaan Bertahap, & Dialog Kasih Sayang"]
        HaditsSiwak["Hadits Siwak: Praktik Mandiri Sebelum Shalat & Memasuki Rumah"] --> Inti
        HaditsPanah["Hadits Memanah: Pendampingan Langsung di Lapangan Latihan"] --> Inti
    end
```
...
```mermaid
flowchart TD
    subgraph SainsDidaktikJasmani["SINTESIS DIDAKTIK OLAHRAGA MODERN"]
        Fitts["1. Fitts & Posner Motor Learning (1967):<br/>• Tahap Kognitif (Paham Teori Gerak)<br/>• Tahap Asosiatif (Latihan & Koreksi Diri)<br/>• Tahap Otonom (Gerak Spontan & Alami)"]
        
        Nudge["2. Nudge Theory (Thaler & Sunstein, 2008):<br/>• Rekayasa Arsitektur Pilihan (Choice Architecture).<br/>• Stimulus visual bawah sadar pengubah perilaku sanitasi."]
        
        Spectrum["3. Spectrum of Teaching Styles (Mosston & Ashworth, 2008):<br/>• Guided Discovery & Self-Check dalam Kebugaran Jasmani."]
        
        Fitts & Nudge & Spectrum ==> DidaktikTUMBUH["DIDAKTIK QAWIYYUL JISM TERPADU"]
    end
```
...
```mermaid
flowchart TD
    subgraph RekayasaNudgeSanitasi["ARSITEKTUR NUDGE SANITASI ASRAMA TUMBUH"]
        N1["1. NUDGE JEJAK KAKI (FOOTPRINT NUDGE):<br/>• Stiker jejak kaki hijau di lantai mengarahkan santri langsung ke tempat sampah & wastafel."]
        N2["2. NUDGE CERMIN KERAPIAN (MIRROR NUDGE):<br/>• Cermin setinggi badan di depan pintu kamar dengan tulisan: 'Apakah Tubuh & Pakaianmu Sudah Bersih & Wangi?'"]
        N3["3. NUDGE AROMATERAPI (SCENT NUDGE):<br/>• Pengharum alami lavender/citrus di lorong asrama yang secara psikologis mendorong santri menjaga kebersihan."]
        
        N1 & N2 & N3 ==> Mandiri["KEPATUHAN HIDUP BERSIH SUKARELA (90%+)"]
    end
```
...
```mermaid
flowchart TD
    subgraph ProtokolDidaktikInklusif["PROTOKOL DIDAKTIK OLAHRAGA INKLUSIF TIER 2"]
        D1["1. PENDEKATAN DIALOGIS GURU PENJAS (GUIDED DISCOVERY)<br/>• Guru Penjas berdialog privat, menjelaskan bahwa olahraga adalah sarana merawat amanah Allah, bukan ajang saling menjatuhkan."]
        
        D2["2. PENGENALAN OLAHRAGA SUNNAH MEMANAH (INDIVIDUAL FOCUS)<br/>• Memperkenalkan panahan yang tidak menuntut lari cepat melainkan fokus ketenangan batin.<br/>• Santri B menunjukkan bakat konsentrasi tinggi & berhasil mengenai target."]
        
        D3["3. PEMBERIAN APRESIASI SPESIFIK & PENGAKUAN SEBAYA<br/>• Guru mengapresiasi ketepatan bidikan Santri B di hadapan teman-temannya."]
        
        D4["4. TRANSISI KE OLAHRAGA KELOMPOK SECARA BERTAHAP<br/>• Santri B mulai percaya diri bergabung dalam senam pagi dan permainan futsal gembira."]
...
```mermaid
flowchart TD
    subgraph EmpatModelDidaktikJasmani["EMPAT MODEL DIDAKTIK QAWIYYUL JISM"]
        M1["1. MODEL MOTOR LEARNING TIGA TAHAP (FITTS & POSNER)<br/>• Kognitif (Pemahaman Anatomi) -> Asosiatif (Drill Korektif) -> Otonom (Refleks Malakah)."]
        
        M2["2. MODEL NUDGE ARCHITECTURE SANITASI (THALER & SUNSTEIN)<br/>• Stimulus visual, penataan fasilitas wastafel otomatis, & dorongan perilaku alami."]
        
        M3["3. MODEL DIDAKTIK OLAHRAGA SUNNAH BERBASIS PENGALAMAN (KOLB)<br/>• Praktik renang/panahan -> Refleksi stamina -> Internalisasi nilai kesatria Islam."]
        
        M4["4. MODEL LIVING HEALTH EXEMPLARITY (KETELADANAN QUDWAH)<br/>• Musyrif & guru menjadi teladan hidup makan sehat, lari bersama, & kamar rapi."]
        
        M1 & M2 & M3 & M4 ==> Keunggulan["PEMBELAJARAN JASMANI TRANSFORMATIF"]
    end
```
...
```mermaid
flowchart TD
    subgraph PanduanNudgeLengkap["PANDUAN OPERASIONAL NUDGE ASRAMA 24 JAM"]
        NudgeKamar["KAMAR TIDUR: Gantungan baju diberi nomor santri; lampu tidur otomatis redup pukul 21.30."]
        NudgeDapur["RUANG MAKAN: Sayur & buah diletakkan di nampan paling depan; dispenser air putih mudah dijangkau."]
        NudgeToilet["KAMAR MANDI: Sabun wangi dispenser otomatis; poster visual adab istinja' di balik pintu bilik."]
        NudgeMasjid["MASJID: Sandal ditata di atas garis stiker saf rapi; rak Al-Qur'an berada pada ketinggian ergonomis."]
        
        NudgeKamar ~~~ NudgeDapur ~~~ NudgeToilet ~~~ NudgeMasjid
    end
```
...
Penerapan konsep **P3-07-12: METODE PEDAGOGI DAN DIDAKTIK JASMANI QAWIYYUL JISM** di ekosistem pesantren TUMBUH memerlukan pemahaman multidimensional yang memadukan khazanah turats syariat dan konsensus sains pendidikan modern:
