...
```mermaid
flowchart TD
    subgraph TransformasiAsesmenKeteraturan["REKONSTRUKSI SISTEM ASESMEN TATA KELOLA HIDUP"]
        SidakTunggal["PENILAIAN SIDAK KAMAR TUNGGAL (TRADISIONAL)<br/>• Sidak seminggu sekali memicu kerapian sandiwara.<br/>• Data keuangan, arsip madrasah, & sarpras terpisah.<br/>• Perilaku ghashab tersembunyi luput dari pantauan."]
        
        TUMBUH["TRIANGULASI TATA KELOLA 360 DERAJAT (TUMBUH)<br/>• Integrasi 5 Sumber: Musyrif, Guru, Sarpras, Mandiri, Sebaya.<br/>• Early Warning System (EWS) mendeteksi ghashab & krisis saku.<br/>• Dashboard Intizham-TUMBUH memantau keteraturan 24 jam."]
        
        SidakTunggal ==>|DIREKONSTRUKSI MENJADI| TUMBUH
    end
```
...
```mermaid
flowchart LR
    subgraph TuratsMuraqabahMasyhudah["PENGAWASAN MULTI-SAKSI DALAM TURATS"]
        Mawardi["Al-Mawardi (Al-Ahkam As-Sulthaniyyah):<br/>Pengawasan Muhtasib Melibatkan Kesaksian Tetangga & Rekan Muamalah"] --> Inti["Al-Muraqabah al-Masyhudah:<br/>Triangulasi Data Keteraturan Hidup Berbasis Bukti Otentik"]
        IbnuTaimiyyah["Ibnu Taimiyyah (Al-Hisbah):<br/>Menghimpun Kesaksian Komunitas untuk Menegakkan Keadilan & Kebersihan Pasar"] --> Inti
        Ghazali["Al-Ghazali (Ihya'):<br/>Hisab Diri Mandiri Disertai Pengawasan Sahabat Karib (Muraqabatul Ikhwan)"] --> Inti
    end
```
...
```mermaid
flowchart TD
    subgraph SainsMTMMKeteraturanHidup["SINTESIS MTMM & VALIDITAS EKOLOGIS"]
        MTMM["Multi-Trait Multi-Method Matrix (Campbell & Fiske, 1959):<br/>• Menguji konvergensi data keteraturan dari metode observasi 5S, self-report kas saku, & rekam sarpras."]
        
        EcologicalValidity["Ecological Momentary Assessment (Stone & Shiffman):<br/>• Pengambilan data pada setting alami kehidupan asrama 24 jam menghilangkan bias kepatuhan semu."]
        
        Dashboard["Intizham-TUMBUH Analytics:<br/>• Algoritma EWS otomatis mendeteksi santri yang berisiko ghashab atau krisis keuangan saku."]
        
        MTMM & EcologicalValidity ==> Dashboard
    end
```
...
```mermaid
flowchart TD
    subgraph AliranDataKeteraturan24Jam["ARSITEKTUR ALIRAN DATA INTIZHAM-TUMBUH"]
        Input1["1. MUSYRIF ASRAMA: Inspeksi Kasur Hotel-Grade, Loker 3 Sekat 5S, & Rak Sandal."]
        Input2["2. GURU MADRASAH: Observasi Meja Belajar Kelas, Portofolio Kitab Bersampul, & Kotak Pensil."]
        Input3["3. TIM SARANA-PRASARANA: Rekam Pemeliharaan Fasilitas Asrama & Ketiadaan Pengrusakan."]
        Input4["4. SANTRI MANDIRI: Buku Kas Saku Harian & Lembar 5S Self-Audit Mandiri."]
        Input5["5. ULASAN SEBAYA: Peer Review: Bebas Ghashab Sandal & Kerapian Jemuran Bersama."]
        
        Input1 & Input2 & Input3 & Input4 & Input5 ==> Server["ENGINE ANALITIK TATA KELOLA (INTIZHAM-TUMBUH)"]
        Server ==> Output1["Rapor Profil Keteraturan & Manajemen Hidup Santri"]
        Server ==> Output2["Early Warning System (EWS) Ghashab & Krisis Finansial"]
        Server ==> Output3["Grafik Kepatuhan Standar 5S Asrama & Madrasah"]
    end
```
...
```mermaid
flowchart TD
    subgraph ProtokolResolusiGhashabDini["PROTOKOL DE-GHASHAB TIER 2"]
        StepA["1. DIALOG RESTORATIF EMPATIK BERSAMA MUSYRIF KAMAR<br/>• Musyrif membedah hadits keharaman ghashab & hak kepemilikan orang lain secara hangat & privat."]
        
        StepB["2. PENGGANTIAN SABUN & PERMINTAAN MAAF KEPADA KAWAN SEKAMAR<br/>• Santri M membeli 2 sabun baru dari uang tabungannya & mengembalikannya kepada kawan sekamar."]
        
        StepC["3. PENEMPATAN SANDAL PADA SLOT RAK BERNOMOR PRIBADI<br/>• Santri M menandai sandalnya dengan spidol permanen & berkomitmen 100% memakai sandal sendiri."]
        
        StepD["4. HASIL INTEGRITAS HAK MILIK MUTLAK (ZERO GHASHAB)<br/>• Dalam 30 hari, Santri M 100% tertib & meraih predikat Kamar Teladan Berintegritas."]
...
```mermaid
flowchart TD
    subgraph LimaPilarTriangulasiKeteraturan["LIMA PILAR TRIANGULASI TATA KELOLA HIDUP 360 DERAJAT"]
        P1["PILAR 1: MUSYRIF ASRAMA (40% Bobot)<br/>• Inspeksi Kasur Hotel-Grade, Loker 3 Sekat 5S, & Rak Sandal Bernomor."]
        
        P2["PILAR 2: GURU MADRASAH (25% Bobot)<br/>• Observasi Meja Belajar Kelas Bebas Coretan & Portofolio Kitab Bersampul."]
        
        P3["PILAR 3: SANTRI MANDIRI (20% Bobot)<br/>• Buku Kas Saku Harian (*Cash-Flow Log*) & Lembar 5S Self-Audit."]
        
        P4["PILAR 4: ULASAN SEBAYA (10% Bobot)<br/>• Peer Review: Bebas Ghashab Sandal/Baju & Kerapian Jemuran Bersama."]
        
        P5["PILAR 5: TIM SARANA-PRASARANA (5% Bobot)<br/>• Catatan Pemeliharaan Sarana Kamar Mandi & Ketiadaan Pengrusakan."]
        
        P1 & P2 & P3 & P4 & P5 ==> Komposit["SKOR INDEKS MUNAZHZHAM FI SYU'UNIH TERPADU (IK-MS)"]
    end
```
...
| :--- | :--- | :--- | :--- |
| **Jenjang J1 (Kelas 7)** | • Merapikan kasur & loker 3 sekat 5S terlabel.<br/>• Sandal di rak nomor & bebas ghashab barang.<br/>• Pencatatan pengeluaran uang saku harian. | • Musyrif: Harian.<br/>• Guru: Mingguan.<br/>• Mandiri: Harian. | Musyrif Kamar J1 & Guru Wali Kelas 7. |
| **Jenjang J2 (Kelas 8–9)** | • Kemandirian mencuci pakaian & sanitasi WC.<br/>• Pos tabungan saku 20% & hemat belanja.<br/>• Perawatan seluruh kitab pelajaran bersampul. | • Musyrif: Pekanan.<br/>• Guru: Mingguan.<br/>• Peer: Bulanan. | Musyrif Asrama & Guru Bidang Studi. |
| **Jenjang J3 (Kelas 10–11)** | • Pengisian lembar *5S Self-Audit* mingguan.<br/>• Perencanaan anggaran bulanan & infaq mandiri.<br/>• Pengorganisasian dokumen & portofolio KTI. | • Mandiri: Pekanan.<br/>• Musyrif: 2 Pekanan.<br/>• Sarpras: Bulanan. | Dosen Pembimbing Halaqah & Musyrif J3. |
| **Jenjang J4 (Kelas 12)** | • Manajemen aset & logistik organisasi OPPM.<br/>• Keteladanan kerapian presisi bagi seluruh asrama.<br/>• Pendampingan standar 5S adik asuh J1. | • Komprehensif: 1x per bulan menjelang kelulusan. | Dewan Pengawas Kepengasuhan & Pembina OPPM. |
