...
```mermaid
flowchart TD
    subgraph TransformasiObservasiKhidmah["REKONSTRUKSI OBSERVASI PERILAKU KEMANFAATAN"]
        Abstrak["PENILAIAN ABSTRAK & BYSTANDER EFFECT (TRADISIONAL)<br/>• Kesalehan dinilai pasif tanpa aksi tolong-menolong.<br/>• Sampah & kran meluap diabaikan oleh santri.<br/>• Eksklusi sosial & pengucilan kawan luput terpantau."]
        
        TUMBUH["KATALOG INDIKATOR TERAMATI 5 SETTING (TUMBUH)<br/>• Setting Kamar, Dapur, Masjid, Kelas, & Desa Binaan.<br/>• BARS (Behaviorally Anchored Rating Scales) terstandar.<br/>• Deteksi dini red flags (Bystander, Ostracism, & Bullying)."]
        
        Abstrak ==>|DIREKONSTRUKSI MENJADI| TUMBUH
    end
```
...
```mermaid
flowchart LR
    subgraph TuratsApresiasiPerilakuKhidmah["APRESIASI PERILAKU KHIDMAH NABAWI"]
        HaditsImathah["HR. Al-Bukhari & Muslim:<br/>إِمَاطَةُ الْأَذَى عَنِ الطَّرِيقِ صَدَقَةٌ<br/>(Menyingkirkan Duri/Gangguan dari Jalan Adalah Sedekah Nyata)"] --> Inti["Hakikat Observasi Perilaku Khidmah:<br/>Merekam Setiap Aksi Kebaikan Nyata yang Memberikan Kemaslahatan Sesama"]
        HaditsMar'ah["HR. Al-Bukhari No. 458:<br/>Nabi SAW Mencari Wanita Pembersih Masjid yang Wafat & Shalat di Atas Kuburnya"] --> Inti
        Ghazali["Al-Ghazali (Ihya'):<br/>Tanda Kematangan Iman Terlihat dari Kelembutan Tangannya Saat Membantu Kawan"] --> Inti
    end
```
...
```mermaid
flowchart TD
    subgraph SainsBARSDanABAPrososial["SINTESIS ABA & BARS PERILAKU PROSOSIAL"]
        BARS["Behaviorally Anchored Rating Scales (Smith & Kendall, 1963):<br/>• Menghubungkan setiap skor penilaian (1-4) dengan contoh tindakan nyata khidmah teramati."]
        
        ABA["Applied Behavior Analysis (Cooper et al., 2020):<br/>• Mengidentifikasi Anteseden (Kawan Sakit/Sampah), Perilaku (Inisiatif Menolong/Pembersihan), & Konsekuensi (Kenyamanan Bersama & Ukhuwah)."]
        
        ProsocialObservables["Prosocial Behavioral Markers (Eisenberg, 2006):<br/>• Perilaku menghibur kawan, berbagi makanan, tutor sebaya, & memimpin kerja bakti."]
        
        BARS & ABA & ProsocialObservables ==> KatalogTUMBUH["KATALOG PERILAKU KHIDMAH TERSTANDAR"]
    end
```
...
```mermaid
flowchart TD
    subgraph LimaSettingEkologiKhidmah["LIMA SETTING OBSERVASI KHIDMAH 24 JAM"]
        S1["SETTING 1: KAMAR TIDUR & ASRAMA (Perawatan Kawan Sakit, Berbagi Makanan, & Piket Kamar)."]
        S2["SETTING 2: DAPUR UMUM & RUANG MAKAN (Piket Saji Makanan, Bersihkan Meja, & Mengalah Antre)."]
        S3["SETTING 3: MASJID & MAJELIS ILMU (Merapikan Shaf/Sandal, Fasilitasi Lansia, & Adab Masjid)."]
        S4["SETTING 4: RUANG KELAS MADRASAH (Tutor Sebaya Belajar, Kerja Kelompok Inklusif, & Papan Tulis)."]
        S5["SETTING 5: MASYARAKAT & DESA BINAAN (Mengajar TPA Ceria, Santun ke Warga, & Bakti Sosial)."]
...
```mermaid
flowchart TD
    subgraph ProtokolEradikasiBystander["PROTOKOL DE-BYSTANDER EFFECT TIER 1 & 2"]
        StepA["1. TINDAKAN KETELADANAN SEKETIKA MUSYRIF: MEMUNGUT SAMPAH SECARA TENANG<br/>• Musyrif tidak memarahi, melainkan langsung berjongkok memungut sampah di depan 10 santri (*Silent Qudwah*)."]
        
        StepB["2. REFLEKSI SOKRATES DI LORONG: 'SIAPA YANG MENDAPATKAN SEDEKAH IMATHATUL ADZA?'<br/>• Musyrif bertanya hangat: 'Akhi, jika Rasulullah lewat, apa yang beliau sabdakan tentang duri/sampah di jalan?' Santri tersadar & malu."]
        
        StepC["3. PENETAPAN 'ATURAN 3 DETIK KHIDMAH LINGKUNGAN' (SEE IT, PICK IT)<br/>• Setiap santri membiasakan aturan otomatis: jika melihat sampah dalam radius 1 meter, wajib dipungut $\le 3\text{ detik}$."]
        
        StepD["4. LAHIRNYA KESADARAN SPONTAN LINGKUNGAN ASRAMA BINTANG LIMA<br/>• Lorong asrama 100% steril dari sampah tercecer & seluruh santri memiliki refleks memungut sampah secara otomatis."]
