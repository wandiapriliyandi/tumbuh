...
Landasan konseptualnya dikembangkan lebih lanjut dalam `01_FOUNDATION`.
...
Definisi dan model formal tentang kapasitas serta progression tidak ditetapkan di dokumen ini. Pembahasan tersebut berada di `02_CORE_MODEL` dan `03_PROGRESSION`.

## 5. Bagaimana Sistem TUMBUH Bekerja?

```text
FOUNDATION
Apa dasar pemikiran TUMBUH?
        ↓
CORE MODEL
Manusia dan kapasitas seperti apa yang dikembangkan?
        ↓
PROGRESSION
Bagaimana pertumbuhan dipahami?
        ↓
ASSESSMENT
Bagaimana perkembangan diketahui?
        ↓
INTERVENTION
Bagaimana sistem merespons kebutuhan perkembangan?
        ↓
IMPLEMENTATION
Bagaimana sistem dijalankan?
        ↓
PROGRAMS
Apa bentuk praktik konkretnya?
```

`08_SOURCES_AND_EVIDENCE` menyediakan sumber dan evidence yang menopang berbagai bagian sistem. `09_RESEARCH` berfungsi untuk menguji, mengevaluasi, dan mengembangkan TUMBUH.

## 6. Arsitektur TUMBUH

| Domain | Pertanyaan utama |
|---|---|
| `01_FOUNDATION` | Apa dasar pemikiran TUMBUH? |
| `02_CORE_MODEL` | Manusia dan kapasitas seperti apa yang ingin dikembangkan? |
| `03_PROGRESSION` | Bagaimana pertumbuhan berlangsung? |
| `04_ASSESSMENT` | Bagaimana pertumbuhan diketahui? |
| `05_INTERVENTION` | Apa yang dilakukan ketika ada kebutuhan perkembangan? |
| `06_IMPLEMENTATION` | Bagaimana sistem dijalankan oleh organisasi? |
| `07_PROGRAMS` | Program apa yang menerjemahkan sistem ke praktik? |
| `08_SOURCES_AND_EVIDENCE` | Apa sumber dan evidence yang menopang sistem? |
| `09_RESEARCH` | Bagaimana sistem diuji dan dikembangkan? |
...
`README.md` → `01_FOUNDATION` → `02_CORE_MODEL`

**Untuk memahami proses pengembangan:**
`02_CORE_MODEL` → `03_PROGRESSION` → `04_ASSESSMENT`

**Untuk menjalankan TUMBUH:**
`02_CORE_MODEL` → `03_PROGRESSION` → `04_ASSESSMENT` → `05_INTERVENTION` → `06_IMPLEMENTATION` → `07_PROGRAMS`

**Untuk meneliti dan mengembangkan TUMBUH:**
`01_FOUNDATION` / `02_CORE_MODEL` → `08_SOURCES_AND_EVIDENCE` → `09_RESEARCH`
