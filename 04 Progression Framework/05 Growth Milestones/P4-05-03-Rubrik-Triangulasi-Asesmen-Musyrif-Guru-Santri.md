...
```mermaid
flowchart TD
    subgraph TransformasiTriangulasiAsesmen["REKONSTRUKSI SISTEM TRIANGULASI ASESMEN"]
        SumberTunggalBias["PENILAIAN TUNGGAL SEPIHAK (TRADISIONAL)<br/>• Guru hanya melihat di kelas; musyrif hanya di kamar.<br/>• Sangat rentan Halo Effect, Horns Effect, & manipulasi sikap.<br/>• Mengabaikan kesaksian sahabat sebaya & refleksi diri."]
        
        TUMBUH["TRIANGULASI ASESMEN 360 DERAJAT (TUMBUH)<br/>• Musyrif (40%) + Guru (20%) + Peer (20%) + Self (20%).<br/>• Multi-Trait Multi-Method (MTMM): Keandalan konstruk teruji.<br/>• Potret karakter 24 jam yang adil, objektif, & transparan."]
        
        SumberTunggalBias ==>|DIREKONSTRUKSI MENJADI| TUMBUH
    end
```
...
```mermaid
flowchart LR
    subgraph TuratsTawatsulSyuhudSalaf["DOKTRIN MULTI-SAKSI DALAM TURATS"]
        HaditsSyuhud["HR. Al-Bukhari No. 1367:<br/>أَنْتُمْ شُهَدَاءُ اللَّهِ فِي الْأَرْضِ<br/>(Kalian Para Sahabat Adalah Saksi-Saksi Allah di Muka Bumi!)"] --> Inti["Hakikat Triangulasi Asesmen:<br/>Memadukan Kesaksian Guru, Pengasuh, & Kawan Sebaya Menghasilkan Keadilan Mutlak"]
        Ghazali["Al-Ghazali (Ihya'):<br/>Tanda Akhlak Mulia Hakiki Adalah Pengakuan Kebaikan dari Kawan Sepergaulan Dekat"] --> Inti
        Syathibi["Asy-Syathibi (Al-Muwafaqat):<br/>Hukum yang Didasarkan pada Banyak Indikasi Saling Menguatkan Menghasilkan Keyakinan Qath'i"] --> Inti
    end
```
...
```mermaid
flowchart TD
    subgraph SainsMTMMDanTriangulasi["SINTESIS MTMM MATRIX & TRIANGULASI ASESMEN"]
        MTMMModel["Multi-Trait Multi-Method (Campbell & Fiske, 1959):<br/>• Menguji Convergent Validity (kesepakatan antar-sumber penilai) & Discriminant Validity (kemampuan membedakan sifat yang berbeda)."]
        
        TriangulationTheory["Methodological Triangulation (Denzin):<br/>• Menggabungkan data observasi kualitatif musyrif, rubrik kuantitatif guru, & sosiometri sebaya."]
        
        BiasReductionAlgorithms["Bias Reduction Mathematical Adjustments:<br/>• Algoritma normalisasi z-score untuk mereduksi rater severity bias & leniency bias."]
        
        MTMMModel & TriangulationTheory & BiasReductionAlgorithms ==> StandarTriangulasiTUMBUH["SISTEM RUBRIK TRIANGULASI 360"]
    end
```
...
```mermaid
flowchart TD
    subgraph AlurSinkronisasiMultiKanal["SINKRONISASI 4 KANAL ASESMEN SIM INTIZHAM"]
        KanalMusyrif["KANAL 1 (40%): Musyrif Kamar mengisi rubrik perilaku asrama 24 jam harian."]
        KanalGuru["KANAL 2 (20%): Guru Madrasah mengisi rubrik adab kelas & ketuntasan belajar."]
        KanalPeer["KANAL 3 (20%): Sahabat Sebaya mengisi evaluasi ukhuwah & kerja tim semesteran."]
        KanalSelf["KANAL 4 (20%): Santri Mandiri mengisi jurnal muhasabah & refleksi diri harian."]
        
        KanalMusyrif & KanalGuru & KanalPeer & KanalSelf ==> EngineKomposit["ENGINE KALKULASI SKOR KOMPOSIT SIM INTIZHAM"]
        EngineKomposit ==> DashboardKarakter["DASHBOARD RADAR KURVA TUMBUH SANTRI 360"]
    end
```
...
```mermaid
flowchart TD
    subgraph ProtokolRekonsiliasiDiskrepansi["PROTOKOL REKONSILIASI ASESMEN TIER 2"]
        Step1["1. SIDANG TRIANGULASI TERBATAS: KONSULOR BK MEMANGGIL MUSYRIF, GURU, & SANTRI W<br/>• Tim menyajikan bukti diskrepansi: Guru melihat adab kelas, musyrif melihat kelalaian 5S kamar."]
        
        Step2["2. KONFRONTASI PENUH KASIH SAYANG: 'ISLAM MENGAJARKAN ADAB LAHIR & BATIN'<br/>• Konselor mengajak Santri W muhasabah: 'Adab sejatimu bukan di depan guru saja nak, melainkan saat di kamar bersama kawanmu'."]
        
        Step3["3. PENETAPAN KONTRAK INTEGRITAS 30 HARI PEMBENAHAN 5S & UKHUWAH KAMAR<br/>• Santri W ditugaskan membersihkan kamar mandi kamar & meminta maaf kepada kawan yang diejeknya."]
        
        Step4["4. HASIL INTEGRASI KARAKTER UTUH (SKOR KOMPOSIT NAIK MENJADI 3.65 - MUMTAZ)<br/>• Sikap kepribadian ganda lenyap; Santri W menjaga adab kelas sekaligus menjadi teladan 5S di kamar."]
...
```mermaid
flowchart TD
    subgraph ArsitekturFormulaKomposit["FORMULA MATEMATIS SKOR KOMPOSIT KARAKTER (SKK)"]
        SMusyrif["SKOR MUSYRIF ASRAMA (40% BOBOT)<br/>• Observasi 24 Jam: Shalat shubuh, 5S kamar, kedisiplinan, & kepemimpinan."]
        
        SGuru["SKOR GURU MADRASAH (20% BOBOT)<br/>• Observasi Kelas: Adab thalabul ilmi, ketekunan belajar, & anti-jidal."]
        
        SPeer["SKOR SAHABAT SEBAYA (20% BOBOT)<br/>• Evaluasi Kamar: Al-Itsar (tidak egois), kejujuran, & solidaritas ukhuwah."]
        
        SSelf["SKOR REFLEKSI SANTRI (20% BOBOT)<br/>• Evaluasi Diri: Kejujuran muhasabah, kesadaran kelemahan, & resolusi perbaikan."]
        
        SMusyrif & SGuru & SPeer & SSelf ==> SKK["SKOR KOMPOSIT TRANSKRIP KARAKTER (SKK: 1.00–4.00)"]
    end
```
...
```mermaid
flowchart LR
    RawData["RAW DATA 4 SUMBER<br/>Logbook Musyrif + Rapor Guru + Sosiometri Peer + Jurnal Self"]
    --> Normalisasi["NORMALISASI Z-SCORE<br/>Mengeliminasi bias rater pelit (Severity) & murah nilai (Leniency)"]
    --> Pembobotan["ALGORITMA PEMBOBOTAN<br/>40% Musyrif + 20% Guru + 20% Peer + 20% Self"]
    --> OutputRadar["VISUALISASI DASHBOARD RADAR 360<br/>Grafik Kurva Pertumbuhan 10 Kapasitas Karakter Santri"]
```
