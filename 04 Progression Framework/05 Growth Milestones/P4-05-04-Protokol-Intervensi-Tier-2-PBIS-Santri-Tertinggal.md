...
```mermaid
flowchart TD
    subgraph TransformasiPenangananTier2["REKONSTRUKSI PENANGANAN SANTRI TERTINGGAL"]
        PublicShaming["PAPAN AIB & HUKUMAN FISIK (TRADISIONAL)<br/>• Memajang aib santri & mempermalukan di depan umum.<br/>• Hukuman tanpa menganalisis akar penyebab masalah.<br/>• Memicu keputusasaan & eskalasi kenakalan santri."]
        
        TUMBUH["INTERVENSI TERARAH TIER 2 PBIS (TUMBUH)<br/>• Menutup Aib (Sitrul 'Urub) & Nasihat Penuh Hikmah.<br/>• Functional Behavior Assessment (FBA) & Behavior Education Program.<br/>• Check-In/Check-Out (CICO) Harian: 100% Santri Pulih."]
        
        PublicShaming ==>|DIREKONSTRUKSI MENJADI| TUMBUH
    end
```
...
```mermaid
flowchart LR
    subgraph TuratsSitrulUyubSalaf["DOKTRIN MENUTUP AIB DALAM TURATS"]
        HaditsSatara["HR. Muslim No. 2699:<br/>مَنْ سَتَرَ مُسْلِمًا سَتَرَهُ اللَّهُ فِي الدُّنْيَا وَالْآخِرَةِ<br/>(Barangsiapa yang Menutup Aib Seorang Muslim, Allah Akan Menutup Aibnya di Dunia & Akhirat!)"] --> Inti["Hakikat Intervensi Tier 2:<br/>Membina Santri Tertinggal Secara Terhormat Tanpa Membuka Aib di Depan Umum"]
        Syafii["Imam Asy-Syafi'i (Diwan):<br/>Nasihat di Depan Umum Adalah Bentuk Pelecehan (Tawbikh), Nasihat Rahasia Adalah Kasih Sayang"] --> Inti
        Fudhail["Al-Fudhail bin 'Iyadh:<br/>Orang Beriman Menasihati Sambil Menutupi Aib; Orang Fasik Mencela Sambil Membuka Aib"] --> Inti
    end
```
...
```mermaid
flowchart TD
    subgraph SainsFBADanTier2PBIS["SINTESIS FBA & BEHAVIOR EDUCATION PROGRAM"]
        FBAModel["Functional Behavior Assessment (O'Neill et al., 1997):<br/>• Menganalisis Anteseden (Pemicu), Perilaku Teramati, & Konsekuensi Penguat (Fungsi: Menghindar atau Mencari Perhatian)."]
        
        BEPCICOSystem["Behavior Education Program / CICO (Crone et al., 2010):<br/>• Pendampingan berbasis penguatan positif harian terstruktur: Rasio 4 Apresiasi : 1 Koreksi."]
        
        SocialSkillsInstruction["Small-Group Social Skills Instruction:<br/>• Melatih regulasi emosi, manajemen amarah, & teknik mengatasi kecemasan belajar."]
        
        FBAModel & BEPCICOSystem & SocialSkillsInstruction ==> StandarTier2TUMBUH["PROTOKOL TIER 2 PBIS"]
    end
```
...
```mermaid
flowchart TD
    subgraph AlurHarianCICO24Jam["RUTINITAS HARIAN CHECK-IN / CHECK-OUT (CICO)"]
        MorningCheckIn["1. MORNING CHECK-IN (05.15 WIB - Ba'da Shubuh): Santri bertemu mentor CICO, menetapkan target DPR-CICO harian, & menerima afirmasi hangat."]
        ClassroomMonitoring["2. DAILY MONITORING SEPANJANG HARI: Guru madrasah & musyrif memberikan feedback skor 1–3 pada lembar DPR di akhir setiap sesi."]
        AfternoonSupport["3. AFTERNOON TUTORING (16.00 WIB): Sesi bantuan belajar/tahfizh 30 menit bersama tutor sebaya J4."]
        EveningCheckOut["4. EVENING CHECK-OUT (21.00 WIB): Santri bertemu mentor CICO malam, merefleksikan capaian skor, & menerima stiker apresiasi."]
        
        MorningCheckIn --> ClassroomMonitoring --> AfternoonSupport --> EveningCheckOut
    end
```
...
```mermaid
flowchart TD
    subgraph ProtokolIntervensiMembolos["PROTOKOL REMEDIASI TIER 2 FBA"]
        Tahap1["1. PEMBATALAN SANKSI GUNDUL & DIALOG EMPATIK BERSAMA KONSELOR BK<br/>• Konselor BK memvalidasi: 'Kami paham kamu takut karena kesulitan nahwu; mari kita selesaikan bersama'."]
        
        Tahap2["2. PENETAPAN MENTOR CICO (USTADZ MUDA PENYABAR) & KONTRAK BELAJAR 30 HARI<br/>• Santri K mengikuti program CICO: Membawa lembar DPR harian untuk memantau fokus belajarnya."]
        
        Tahap3["3. BANTUAN SCAFFOLDING BELAJAR 30 MENIT SORE DENGAN METODE FLASHCARD NAHWU<br/>• Ustadz mendampingi Santri K membedah rumus tashrif & i'rab secara visual dan menyenangkan."]
        
        Tahap4["4. HASIL PEMULIHAN KEPERCAYAAN DIRI (SKOR DPR 92% & KEHADIRAN BELAJAR 100%)<br/>• Perilaku bersembunyi hilang total; Santri K aktif duduk di meja belajar & meraih nilai B+ pada ujian nahwu."]
...
```mermaid
flowchart TD
    subgraph ArsitekturTier2PBIS["EMPAT PILAR INTERVENSI TERARAH TIER 2 PBIS"]
        P1["PILAR 1: PENAPISAN DATA EWS OTOMATIS<br/>• Santri yang tidak mencapai $70\%$ target milestone otomatis dirujuk ke tim Tier 2 PBIS."]
        
        P2["PILAR 2: CHECK-IN / CHECK-OUT (CICO) HARIAN<br/>• Sesi 5 menit pagi & malam bersama mentor pendamping untuk menjaga motivasi berprestasi."]
        
        P3["PILAR 3: ACADEMIC & BEHAVIORAL SCAFFOLDING<br/>• Bantuan tutor sebaya, klinik tahfizh terarah, & workshop regulasi emosi."]
        
        P4["PILAR 4: EVALUASI KELULUSAN & TRANSISI KEMBALI KE TIER 1<br/>• Setelah 30–60 hari konsisten meraih skor DPR $\ge 80\%$, santri dinyatakan tuntas & kembali mandiri."]
