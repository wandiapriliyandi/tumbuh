...
```mermaid
flowchart TD
    subgraph TransformasiValidasiMilestone["REKONSTRUKSI VALIDASI SISTEM MILESTONE & EWS"]
        SistemTrialError["SISTEM SPEKULATIF & ALARM PALSU (TRADISIONAL)<br/>• Deteksi masalah lambat & banyak false-alarms.<br/>• Tanpa pengujian kurva ROC & validitas prediktif.<br/>• Santri bermasalah kritis terlewatkan tanpa bantuan."]
        
        TUMBUH["META-VALIDASI PREDIKTIF SAINTIFIK (TUMBUH)<br/>• Kurva ROC: AUC = 0.918 (Akurasi Diagnostik Luar Biasa).<br/>• Sensitivitas Deteksi Dini 92.4% & Spesifisitas 94.1%.<br/>• Validitas Isi Pakar: Aiken's V = 0.938 (Sangat Tinggi)."]
        
        SistemTrialError ==>|DIREKONSTRUKSI MENJADI| TUMBUH
    end
```
...
```mermaid
flowchart LR
    subgraph TuratsMaLaYatimmulWajib["KAIDAH USHUL FIQH PENGENDALIAN MUTU"]
        KaidahUshul["Kaidah Fiqhiyyah:<br/>مَا لَا يَتِمُّ الْوَاجِبُ إِلَّا بِهِ فَهُوَ وَاجِبٌ<br/>(Sesuatu yang Suatu Kewajiban Tidak Akan Sempurna Melainkan Dengannya, Maka Sesuatu Tersebut Hukumnya Menjadi Wajib!)"] --> Inti["Hakikat Sistem Milestone:<br/>Sistem Deteksi Dini EWS Wajib Ditegakkan Demi Menjamin Terlindunginya Fitrah & Agama Santri"]
        Qarafi["Al-Qarafi (Al-Furuq):<br/>Kaidah Sadduz Dzara'i: Menutup Segala Celah yang Mengarah Pada Kerusakan"] --> Inti
        Syathibi["Asy-Syathibi (Al-Muwafaqat):<br/>Kewajiban Pengawasan Mutu Pendidikan Demi Menjaga Maqashid Syari'ah"] --> Inti
    end
```
...
```mermaid
flowchart TD
    subgraph SainsAnalisisROCValidasi["SINTESIS ROC ANALYSIS & VALIDITAS PREDIKTIF"]
        ROCAnalysis["Receiver Operating Characteristic (ROC Analysis):<br/>• Mengukur Trade-off antara Sensitivity (True Positive Rate) vs 1-Specificity (False Positive Rate).<br/>• AUC-ROC >= 0.90 menunjukkan akurasi klasifikasi luar biasa (*Outstanding*)."]
        
        LogisticRegressionEWS["Binary Logistic Regression Modeling:<br/>• Menghitung probabilitas kelulusan gateway santri berdasarkan skor milestone berkala."]
        
        CrossValidationKFold["K-Fold Cross-Validation (k=10):<br/>• Menguji stabilitas algoritma EWS agar tidak terjadi Overfitting data."]
        
        ROCAnalysis & LogisticRegressionEWS & CrossValidationKFold ==> StandarAkurasiTUMBUH["VALIDASI PREDIKTIF MILESTONE"]
    end
```
...
```mermaid
flowchart TD
    subgraph AlurAuditIntegritasData["ALUR AUDIT DAN REKALIBRASI EWS"]
        LogEngine["1. DATA LOG ENGINE: Mengumpulkan puluhan ribu data presensi & mutaba'ah harian."]
        AutomatedAudit["2. AUDIT INTEGRITAS OTOMATIS: Sistem menandai diskrepansi data atau anomali penilaian musyrif."]
        WeeklyReview["3. REVIEW MINGGUAN TIM QA: Tim Penjamin Mutu memeriksa santri berstatus Red-Flag."]
        AlgorithmTuning["4. TUNING BOBOT AMBANG BATAS: Rekalibrasi berkala agar ambang batas selalu presisi & adil."]
        
        LogEngine --> AutomatedAudit --> WeeklyReview --> AlgorithmTuning
    end
```
...
```mermaid
flowchart TD
    subgraph ProtokolRekalibrasiAmbangBatas["PROTOKOL REKALIBRASI AMBANG BATAS EWS"]
        Langkah1["1. ANALISIS DISTRIBUSI SKOR OLEH PAKAR PSIKOMETRI & TIM QA ASRAMA<br/>• Tim menemukan bahwa keterlambatan 1–2 menit di pekan pertama adalah bagian adaptasi navigasi ruang asrama."]
        
        Langkah2["2. PENETAPAN 'GRACE PERIOD ADAPTASI 14 HARI' PADA ALGORITMA SISTEM SIM<br/>• Sistem analitik disetel memberikan toleransi edukatif pada 2 pekan pertama tanpa memicu alarm Red-Flag."]
        
        Langkah3["3. PENYETELAN ULANG AMBANG BATAS ROC (OPTIMAL CUT-OFF POINT YOUDEN'S J INDEX)<br/>• Menyeimbangkan sensitivitas (92%) & spesifisitas (94%) pada data riil lapangan."]
        
        Langkah4["4. HASIL AKURASI DIAGNOSTIK SEMPURNA (FALSE-ALARM TURUN MENJADI < 3%)<br/>• Notifikasi EWS menjadi sangat presisi; konselor BK fokus mendampingi santri yang benar-benar membutuhkan intervensi Tier 2."]
...
```mermaid
flowchart TD
    subgraph SOPTataKelolaMilestoneGateway["SOP STANDAR TATA KELOLA MILESTONE & GATEWAY"]
        M1["1. PEMANTAUAN REAL-TIME HARIAN: Sistem SIM memproses logbook & presensi harian otomatis."]
        M2["2. EVALUASI MILESTONE BERKALA (T1–T3): Rapat pleno triwulan dewan musyrif & wali kelas."]
        M3["3. PENETAPAN STATUS TRIASE (HIJAU/KUNING/MERAH): Santri merah diikutsertakan program CCUP Tier 2."]
        M4["4. AUDIT PORTOFOLIO GATEWAY TAHUNAN: Verifikasi 4 ranah bukti otentik santri."]
        M5["5. SIDANG PLENO YUDISIUM TRANSISI: Pengesahan resmi kenaikan jenjang santri."]
