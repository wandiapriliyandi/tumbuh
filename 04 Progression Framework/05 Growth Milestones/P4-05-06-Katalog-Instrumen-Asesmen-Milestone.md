...
```mermaid
flowchart TD
    subgraph TransformasiKatalogInstrumen["REKONSTRUKSI INSTRUMEN ASESMEN ASRAMA"]
        FormatAcakAmbigu["CATATAN ACAK & DESKRIPTOR AMBIGU (TRADISIONAL)<br/>• Format penilaian berbeda-beda antar-musyrif.<br/>• Tanpa deskriptor perilaku eksplisit per skor.<br/>• Fluktuasi nilai tinggi & tidak dapat dipertanggungjawabkan."]
        
        TUMBUH["KATALOG INSTRUMEN BAKU TUMBUH (TERSTANDARISASI)<br/>• 6 Instrumen Lengkap: SKA-F1, LMK-F2, SKE-F3, RKK-F4, BAK, DPR.<br/>• Deskriptor analitik eksplisit dengan rubrik 4 level.<br/>• Test-Retest Reliability: r >= 0.89 (Konsistensi Sangat Tinggi)."]
        
        FormatAcakAmbigu ==>|DIREKONSTRUKSI MENJADI| TUMBUH
    end
```
...
```mermaid
flowchart LR
    subgraph TuratsMizanQisthasSalaf["DOKTRIN TIMBANGAN ADIL TURATS"]
        AyatMizan["QS. Al-Isra' [17]: 35:<br/>وَأَوْفُوا الْكَيْلَ إِذَا كِلْتُمْ وَزِنُوا بِالْقِسْطَاسِ الْمُسْتَقِيمِ ذَلِكَ خَيْرٌ وَأَحْسَنُ تَأْوِيلًا<br/>(Dan Sempurnakanlah Takaran Apabila Kalian Menakar & Timbanglah dengan Neraca yang Lurus!)"] --> Inti["Hakikat Standarisasi Instrumen:<br/>Menyediakan Alat Ukur Evaluasi yang Lurus, Adil, & Presisi Demi Keadilan Santri"]
        Ghazali["Al-Ghazali (Al-Qistas Al-Mustaqim):<br/>Neraca Akal & Syariat Harus Terbebas dari Cacat Subjektivitas"] --> Inti
        IbnQayyim["Ibnu Qayyim (I'lamul Muwaqqi'in):<br/>Menilai Amal Wajib Menggunakan Timbangan Keadilan Syar'i"] --> Inti
    end
```
...
```mermaid
flowchart TD
    subgraph SainsStandarisasiInstrumen["SINTESIS STANDAR PSIKOMETRI INSTRUMEN"]
        TestRetestReliability["Test-Retest Reliability (r >= 0.85):<br/>• Menguji stabilitas skor penilaian yang diberikan musyrif pada rentang 7 hari tanpa intervensi."]
        
        SystemUsabilityScale["System Usability Scale (SUS >= 80):<br/>• Menguji kemudahan pengisian form oleh musyrif kamar (User-Centered Design)."]
        
        BehaviorallyAnchored["Behaviorally Anchored Rating Scales (BARS):<br/>• Menyediakan contoh perilaku nyata di setiap poin skala untuk memandu penilai."]
        
        TestRetestReliability & SystemUsabilityScale & BehaviorallyAnchored ==> StandarKatalogTUMBUH["KATALOG INSTRUMEN BAKU"]
    end
```
...
```mermaid
flowchart TD
    subgraph AlurDigitalInstrumen24Jam["ALUR DIGITALISASI INSTRUMEN ASESMEN"]
        AppMusyrif["1. INPUT CEPAT MOBILE APP: Musyrif mengisi form via smartphone (3 menit per kamar)."]
        AutoValidation["2. VALIDASI OTOMATIS: Sistem memastikan tidak ada butir yang terlewat."]
        CloudSync["3. SINKRONISASI CLOUD: Data tersimpan aman di server pusat & terenkripsi."]
        ReportGeneration["4. GENERASI LAPORAN REAL-TIME: Santri & orang tua dapat memantau grafik perkembangan."]
        
        AppMusyrif --> AutoValidation --> CloudSync --> ReportGeneration
    end
```
...
```mermaid
flowchart TD
    subgraph ProtokolSertifikasiMusyrif["PROTOKOL SERTIFIKASI PENILAI TIER 1"]
        TahapA["1. COACHING SATU-PADA-SATU OLEH KEPALA PENGASUHAN & AUDITOR QA<br/>• Kepala Pengasuhan menegaskan: 'Form SKA-F1 adalah alat bantu untuk menolong santri, bukan ajang kompetisi antar-kamar'."]
        
        TahapB["2. BEDAH BUKU MANUAL KATALOG & SIMULASI PENILAIAN 5 KASUS VIGNETTE<br/>• Ustadz R dilatih mencocokkan indikator deskriptor BARS dengan perilaku riil santri di kamar."]
        
        TahapC["3. ASISTENSI PENILAIAN BERSAMA (CO-RATING) SELAMA 7 HARI BERSAMA MUSYRIF SENIOR<br/>• Melakukan observasi bersama hingga Ustadz R percaya diri menentukan skor secara objektif."]
        
        TahapD["4. HASIL SERTIFIKASI MUSYRIF PENILAI & AKURASI DATA ASESMEN TERJAMIN 100%<br/>• Ustadz R lulus uji kompetensi penilai; 4 santri homesickness teridentifikasi & sukses didampingi."]
...
```mermaid
flowchart TD
    subgraph EnamInstrumenBakuTUMBUH["KATALOG ENAM INSTRUMEN ASESMEN BAKU TUMBUH"]
        F1["1. FORM SKA-F1: SKALA KESIAPAN ADAPTASI INISIASI (JENJANG J1)<br/>• Mengukur tuntasnya homesickness, kemandirian mencuci pakaian, & adaptasi asrama."]
        
        F2["2. FORM LMK-F2: LEMBAR MUTABA'AH KONSISTENSI HABITUASI (JENJANG J2)<br/>• Mengukur otomatisasi shalat berjamaah di masjid, standar 5S kamar, & P3K dasar."]
        
        F3["3. FORM SKE-F3: SKALA KEMATANGAN SOSIAL-EMOSIONAL SEL (JENJANG J3)<br/>• Mengukur 5 kompetensi CASEL SEL, de-polarisasi geng, & empati tutor adik asuh."]
        
        F4["4. FORM RKK-F4: RUBRIK KINERJA KEPEMIMPINAN QUDWAH (JENJANG J4)<br/>• Mengukur kepemimpinan pelayan (Servant Leadership OPPM) & Capstone Project."]
        
        F5["5. FORM BAK-GATEWAY: BERITA ACARA KELULUSAN GATEWAY TRANSISI (G1–G3)<br/>• Dokumen pengesahan resmi kenaikan jenjang berbasis portofolio 4 ranah."]
        
        F6["6. FORM DPR-CICO: LEMBAR KEMAJUAN HARIAN INTERVENSI TIER 2 PBIS<br/>• Instrumen pemantauan terarah harian bagi santri yang memerlukan pendampingan."]
        
        F1 & F2 & F3 & F4 & F5 & F6 ==> SIMIntizhamPortofolio["PORTOFOLIO DIGITAL TERPADU SANTRI"]
    end
```
...
```mermaid
flowchart LR
    Langkah1["1. OBSERVASI HARIAN<br/>Musyrif mengamati perilaku santri secara natural tanpa mengintimidasi."]
    --> Langkah2["2. PENSKORAN BARS<br/>Mencocokkan perilaku teramati dengan deskriptor rubrik 1, 2, 3, atau 4."]
    --> Langkah3["3. INPUT SIM MOBILE<br/>Memasukkan skor via aplikasi Android/iOS SIM Intizham dalam 3 menit."]
    --> Langkah4["4. AUDIT QA OTOMATIS<br/>Sistem menghitung agregat skor & menghasilkan notifikasi triase otomatis."]
```
