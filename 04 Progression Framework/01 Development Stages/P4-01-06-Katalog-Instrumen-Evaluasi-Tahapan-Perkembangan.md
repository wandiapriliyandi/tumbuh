...
```mermaid
flowchart TD
    subgraph TransformasiInstrumenPerkembangan["REKONSTRUKSI INSTRUMEN EVALUASI PERKEMBANGAN"]
        UjianKertasSaja["EVALUASI KOGNITIF KERTAS SEMATA (TRADISIONAL)<br/>• Kenaikan jenjang hanya berdasarkan rapor madrasah.<br/>• Kematangan emosi, kemandirian, & kepemimpinan diabaikan.<br/>• Tanpa pelacakan rekam jejak pertumbuhan longitudinal."]
        
        TUMBUH["PAKET MASTER INSTRUMEN 4 ETAPE TUMBUH<br/>• Form SKA-F1: Kesiapan Adaptasi Inisiasi (J1).<br/>• Form LMK-F2: Konsistensi Habituasi 66 Hari (J2).<br/>• Form SKE-F3: Kematangan Sosial-Emosional SEL (J3).<br/>• Form RKK-F4: Servant Leadership & Capstone Project (J4)."]
        
        UjianKertasSaja ==>|DIREKONSTRUKSI MENJADI| TUMBUH
    end
```
...
```mermaid
flowchart LR
    subgraph TuratsImtihanAdabSalaf["PENGUJIAN ADAB & KEMATANGAN SALAF"]
        HaditsImtihan["Shahih Al-Bukhari (Kitab Al-Ilm):<br/>Nabi SAW Menguji Kematangan Nalar & Adab Para Sahabat Melalui Pertanyaan Pohon Kurma"] --> Inti["Hakikat Asesmen Perkembangan:<br/>Menguji Kesiapan Batiniah, Ketajaman Nalar, & Kestabilan Akhlak Murid"]
        KhatibBaghdadi["Al-Khatib Al-Baghdadi (Al-Faqih wal Mutafaqqih):<br/>Syarat Kelulusan: Ujian Integritas Wara', Kesabaran Khidmah, & Penguasaan Ilmu"] --> Inti
        Ghazali["Al-Ghazali (Ihya'):<br/>Pengujian Murid Melalui Penugasan Nyata di Lapangan Sebelum Diberi Amanah Besar"] --> Inti
    end
```
...
```mermaid
flowchart TD
    subgraph SainsPsikometriInstrumenEtape["SINTESIS STANDAR PSIKOMETRI ETAPE"]
        PsychometricReliability["Internal Consistency (Cronbach's Alpha >= 0.88):<br/>• Konsistensi pengukuran butir skala adaptasi, habituasi, SEL, & leadership."]
        
        ConstructValidity["Construct & Criterion Validity (Aiken's V >= 0.90):<br/>• Validitas isi teruji oleh dewan pakar psikologi perkembangan & pengasuhan."]
        
        GatewayAssessment["Gateway Competency Model (Spady, 1994):<br/>• Ambang batas kelulusan prasyarat sebelum santri melangkah ke fase berikutnya."]
        
        PsychometricReliability & ConstructValidity & GatewayAssessment ==> MasterInstrumenTUMBUH["EMPAT MASTER INSTRUMEN ETAPE"]
    end
```
...
```mermaid
flowchart TD
    subgraph AlurDataEvaluasiEtape["ALUR EVALUASI PERKEMBANGAN LONGITUDINAL"]
        F1["INPUT FORM SKA-F1: Musyrif Kamar (Akhir Bulan Ke-1 / Hari Ke-40)."]
        F2["INPUT FORM LMK-F2: Musyrif & Santri (Akhir Siklus 66 Hari Habituasi)."]
        F3["INPUT FORM SKE-F3: Konselor BK & Guru (Akhir Semester Jenjang J3)."]
        F4["INPUT FORM RKK-F4: Dewan Penguji Capstone & Kepala Asrama (Akhir J4)."]
        
        F1 & F2 & F3 & F4 ==> EngineAnalitik["ENGINE PENGOLAH DATA SIM INTIZHAM-TUMBUH"]
        EngineAnalitik ==> RadarChart["DIAGRAM RADAR 360 DERAJAT KURVA TUMBUH SANTRI"]
    end
```
...
```mermaid
flowchart TD
    subgraph ProtokolRemediasiGateway2["PROTOKOL REMEDIASI ETAPE 2 (TIER 2 CATCH-UP)"]
        Tahap1["1. PENAHANAN TRANSISI TANGGA KE J3 & FORUM DIALOG WALI ASRAMA<br/>• Musyrif menjelaskan secara hangat: 'Kecerdasan akalmu luar biasa, namun pondok ini mendidik adab hidup; mari kita rapikan kebiasaan harianmu'."]
        
        Tahap2["2. PROGRAM 'BOOTCAMP HABITUASI 30 HARI' BERSAMA MUSYRIF SENIOR<br/>• Santri B dibimbing merapikan lemari kamar dengan standar 5S & didampingi bangun shubuh secara konsisten."]
        
        Tahap3["3. PENUGASAN KHIDMAH PIKET MEJA MAKAN & MURAJA'AH MANDIRI<br/>• Melatih tanggung jawab fisik melayani makan santri & mengulang hafalan di meja belajar tepat waktu."]
        
        Tahap4["4. HASIL RE-EVALUASI GATEWAY DENGAN PRESTASI PARIPURNA<br/>• Skor LMK melonjak menjadi $3.75$, kamar rapi bintang lima, & Santri B resmi dilantik naik ke Jenjang J3 dengan penuh kebanggaan."]
