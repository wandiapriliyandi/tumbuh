...
```mermaid
flowchart TD
    subgraph SarprasBuruk["SARPRAS BURUK & ABORSI ADAB"]
        F1["Kamar Berdesakan (20 Santri/Bilik)"] --> E1["Spatial Crowding & Amigdala Tegang"]
        F2["Lorong Gelap & Blind Spot"] --> E2["Terbentuk Celah Bullying & Asusila"]
        F3["Fasilitas Rusak Tak Diperbaiki"] --> E3["Broken Windows Effect (Kekacauan Budaya)"]
    end

    subgraph SarprasTumbuh["SARPRAS TUMBUH (BI'AH SHALIHAH)"]
        G1["Kamar 5S Ergonomis (6-8 Santri/Bilik)"] --> S1["Kualitas Tidur 7 Jam Terjamin (PFC Bugar)"]
        G2["CPTED (Zero Blind Spots)"] --> S2["Lingkungan Aman Transparan (Self-Restraint)"]
        G3["Perbaikan Cepat <24 Jam"] --> S3["Kultur Memuliakan & Menjaga Fasilitas"]
    end
```
...
```mermaid
flowchart LR
    Kurikulum["<b>1. KURIKULUM</b><br/><i>Merancang Nilai & Adab</i><br/>(Silabus 10 Muwashafat, Asesmen Ipsatif, KBM Terpadu)"]
    
    Kesiswaan["<b>2. KESISWAAN</b><br/><i>Membiasakan & Menjaga Afeksi</i><br/>(Musyrif 24 Jam, CICO Tier 2, Ishlah Restoratif 3R)"]
